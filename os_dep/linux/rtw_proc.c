...
static int proc_get_ch_sel_policy(struct seq_file *m, void *v)
{
	...
	RTW_PRINT_SEL(m, "%-16s %-9s %-7s\n", "within_same_band", "pref_band", "pref_ch");

	RTW_PRINT_SEL(m, "%16d %9s %7d\n", rfctl->ch_sel_within_same_band
		, band_str(rfctl->ch_sel_pref_band), rfctl->ch_sel_pref_ch);
...
static ssize_t proc_set_ch_sel_policy(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	...
	num = sscanf(tmp, "%hhu %hhd %hhu", &within_sb, &pref_band, &pref_ch);
	if (num >= 1 && within_sb < RTW_CHSEL_BAND_NUM)
		rfctl->ch_sel_within_same_band = within_sb;
	if (num >= 2) {
		if (pref_band < 0 || pref_band > BAND_MAX)
			pref_band = BAND_MAX;
		rfctl->ch_sel_pref_band = pref_band;
	}
	if (num >= 3)
		rfctl->ch_sel_pref_ch = pref_ch;
...
static ssize_t proc_set_p2p_park_ch(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	...
	if (ch != 36 && ch != 149)
		...
		rfctl->ch_sel_pref_band = BAND_ON_5G;
		rfctl->ch_sel_pref_ch = ch;
	}
...
#ifdef PRIVATE_R
	RTW_PROC_HDL_SSEQ("wow_wake_reason", proc_get_wakeup_reason, NULL),
#endif /* PRIVATE_R */
