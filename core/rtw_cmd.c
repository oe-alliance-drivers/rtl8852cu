...
static u8 rtw_createbss_cmd(_adapter  *adapter, int flags, bool adhoc
	, u8 ifbmp, u8 excl_ifbmp, struct bss_ch_sel_req ch_sel, u8 is_change_chbw)
{
	...
	if (ch_sel.band >=0 && ch_sel.ch > 0 && ch_sel.bw >= 0 && ch_sel.offset >= 0) {
		if (!rtw_chset_is_bchbw_valid(adapter_to_chset(adapter), ch_sel.band, ch_sel.ch, ch_sel.bw, ch_sel.offset, 0, 0)) {
			res = _FAIL;
			goto exit;
		}
	}
...
{
	struct bss_ch_sel_req ch_sel = DUMMY_BSS_CH_SEL_REQ;
...
{
	struct bss_ch_sel_req ch_sel = {req_band, req_ch, req_bw, req_offset};
