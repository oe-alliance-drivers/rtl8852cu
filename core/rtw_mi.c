					...
					union recv_frame *precvframe)
{
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if (type == WIFI_DATA_TYPE && !adapter_allow_bmc_data_rx(GET_PRIMARY_LINK(iface)))
			continue;
...
struct _ADAPTER_LINK *rtw_get_alink_by_iflidx(struct dvobj_priv *dvobj, u8 iflidx)
{
	/* this driver has single link only */
	if (iflidx < dvobj->iface_nums) {
		_adapter *a = dvobj->padapters[iflidx];
		if (a && rtw_is_adapter_up(a))
			return GET_PRIMARY_LINK(a);
	}
