...
static void parsing_country_element_ie(struct link_mlme_ext_priv *pmlmeext, u8* ie, s16 len)
{
	...
	while (len - 3 >= 0) {
		/* bypass operating triplet field */
		if (*ie > 200) {
			ie += 3;
			len -= 3;
			continue;
...
unsigned int OnAssocReq(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (padapter->mlmepriv.smeofld_ap) {
		...
		rtw_cfg80211_rx_mgmt(padapter->rtw_wdev,
				     rtw_bch2freq(lmlmeextpriv->chandef.band, lmlmeextpriv->chandef.chan),
...
unsigned int OnDeAuth(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (pmlmepriv->smeofld_ap ||
		...
		if (pmlmepriv->smeofld_ap) {
			rtw_cfg80211_rx_mgmt(padapter->rtw_wdev,
					     rtw_bch2freq(lpriv->chandef.band, lpriv->chandef.chan),
...
void rtw_disconnect_ch_switch(_adapter *adapter)
{
	...
	if (!MLME_IS_OPCH_SW(adapter)) {
		...
		if (MLME_IS_STA(adapter)) {
			...
			rtw_dfs_rd_en_dec_on_mlme_act(adapter_link, MLME_STA_DISCONNECTED, 0);
			#endif
		}
...
#define DUMP_ADAPTERS_STATUS 0
	...
	for (lidx = 0; lidx < adapter->adapter_link_num; lidx++) {
		...
		if (join_res >= 0 && IS_ACCEPTED_LINK(adapter_link)) { /*client join success - restart all ap*/
			...
			#if CONFIG_DFS && CONFIG_IEEE80211_BAND_5GHZ
			rtw_dfs_rd_en_dec_on_mlme_act(adapter_link, MLME_STA_CONNECTED, 0);
			#endif
