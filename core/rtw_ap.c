...
u8 rtw_ap_find_mini_tx_rate(struct _ADAPTER_LINK *adapter_link)
{
	_adapter *adapter = adapter_link->adapter;
	_list	*phead, *plist;
	...
	while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
		...
		if (psta->padapter_link != adapter_link)
			continue;
...
void rtw_update_bmc_sta_tx_rate(_adapter *adapter)
{
	...
	if (!rtw_linked_check_alink(adapter_link))
		goto _exit;

	tx_rate = rtw_ap_find_mini_tx_rate(adapter_link);
...
void rtw_core_ap_swch_start(_adapter *padapter, struct createbss_parm *parm)
{
	...
	if (parm->ch_to_set != 0) {
		set_bch_bwmode(padapter, padapter_link, parm->band_to_set,
...
void rtw_core_ap_chan_decision(_adapter *padapter, struct createbss_parm *parm)
{
	...
	if (!(parm->ch_sel.ch == 0 && padapter->mlmeextpriv.mlmext_info.state == WIFI_FW_AP_STATE))
		return;
...
chbw_decision:
	...
	ifbmp_ch_changed = rtw_ap_bchbw_decision(padapter
		, parm->ifbmp, parm->excl_ifbmp, &parm->ch_sel, &chdef_to_set);
	...
	rtw_dfs_rd_en_dec_on_mlme_act(padapter_link, mlme_act, parm->excl_ifbmp);
#endif
...
_exit:
	...
	parm->band_to_set = new_chdef.band;
		...
		u8 ie_id, u8 *oui, u8 tx, u8 flags, const char *tag)
{
	...
	case WLAN_EID_COUNTRY:
		rtw_rfctl_sync_country_ie(adapter_to_rfctl(padapter)
			, ALINK_GET_BAND(padapter_link)
			, BSS_EX_IES(&(pmlmeext->mlmext_info.network))
			, &BSS_EX_IES_LEN(&(pmlmeext->mlmext_info.network))
			, _BEACON_IE_OFFSET_);
		break;
#endif
...
#ifdef CONFIG_80211D
	rtw_rfctl_sync_country_ie(adapter_to_rfctl(adapter), chandef->band
		, BSS_EX_IES(bss), &BSS_EX_IES_LEN(bss), _BEACON_IE_OFFSET_);
#endif
	...
	, const char *caller)
{
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if (MLME_IS_ASOC(iface)
			...
			if (rtw_linked_check_alink(iface_link) == _TRUE) {
				#ifdef CONFIG_ECSA_PHL
	...
	, const struct bss_ch_sel_req *req, struct rtw_chan_def *chdef)
{
	...
	RTW_INFO("%s ifbmp:0x%02x excl_ifbmp:0x%02x req:%d,%d,%d,%d\n", __func__
		, ifbmp, excl_ifbmp, req->band, req->ch, req->bw, req->offset);
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if (req->ch == 0) {
			...
			dec_offset[i] = (req->offset <= REQ_OFFSET_NONE) ? cur_ie_offset[i] : req->offset;
			#ifdef CONFIG_DBCC_SUPPORT
	...
	if (MSTATE_STA_LG_NUM(&mstate_others) || MSTATE_STA_LD_NUM(&mstate_others)
		...
		if (req->ch <= REQ_CH_NONE) /* channel is not specified */
			goto choose_chbw;
		...
		if (rtw_ap_ch_specific_chk(adapter, u_band, u_ch, &u_bw, &u_offset, __func__) == _FAIL) {
			...
			if (req->ch > 0 && req->bw > REQ_BW_NONE) {
...
choose_chbw:
		ch_avail = rtw_choose_bss_bchbw_iflbmp(dvobj, ALINK_GET_HWBAND(adapter_link)
			, IFBMP_TO_IFLBMP(ifbmp), req, &u_band, &u_ch, &u_bw, &u_offset, __func__);
		if (ch_avail == _FALSE)
			goto exit;
...
static enum phl_mdl_ret_code _ap_start_req_ev_hdlr(void *dispr, void *priv, struct phl_msg *msg)
{
	...
	case MSG_EVT_AP_START_END:
		...
		if (*(msg->inbuf) == RTW_PHL_STATUS_FAILURE){
			...
			rtw_p2p_group_if_pd_sync(padapter, PHL_CMD_DIRECTLY, DBCC_CHK_AP_START);
		}
		#endif
