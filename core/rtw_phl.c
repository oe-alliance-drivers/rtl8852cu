...
_search_adapter_by_rtype(struct _ADAPTER *a, u8 band_idx, enum dbcc_rtype rtype)
{
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if (rtype == DBCC_RTYPE_P2P_PD) {
			if (MLME_IS_PD(iface)) {
				tg_apt = iface;
				break;
			}
		}
		else if (rtype == DBCC_RTYPE_P2P) {
...
void rtw_p2p_group_if_pd_sync(_adapter *adapter,
			     ...
			     enum dbcc_chk_pcd chk_pcd)
{
	...
	struct rtw_wifi_role_link_t *rlink = alink->wrlink;
	enum phl_band_idx band_idx = rlink->hw_band;

	if (!rtw_phl_mr_is_db(dvobj->phl))
		return;
	if (dvobj->iface_nums != 3) /*for p2p-group interface only*/
		return;
	...
	case DBCC_CHK_AP_START:
	{
		...
		struct _ADAPTER_LINK *pd_alink = NULL;
		...
		pd_adp = _search_adapter_by_rtype(adapter, HW_BAND_MAX, DBCC_RTYPE_P2P_PD);
		if (pd_adp) {
			pd_alink = GET_PRIMARY_LINK(pd_adp);
			#ifdef DBG_DBCC_LG_SCENARIO
			RTW_INFO(FUNC_ADPT_FMT"- P2P-GO at HW_B(%d)\n",
				FUNC_ADPT_ARG(adapter),
				rlink->hw_band);
			RTW_INFO(FUNC_ADPT_FMT" - P2P-PD at HW_B(%d)\n",
				FUNC_ADPT_ARG(pd_adp),
				pd_alink->wrlink->hw_band);
			#endif
			if (pd_alink->wrlink->hw_band != band_idx)
				rtw_phl_wifi_role_realloc_band(dvobj->phl, pd_adp->phl_role, pd_alink->wrlink);
		} else {
			RTW_ERR("%s cannot find P2P-PD\n", __func__);
		}
	}
	break;
...
static const hrate halrs_of_byrate_offset[] = {
	HAL_RS_CCK,
	HAL_RS_OFDM,
	HAL_RS_HT_1SS,		/* for all HT */
	HAL_RS_VHT_1SS,		/* for all VHT */
	HAL_RS_HE_1SS,		/* for all HE */
#if RTW_PHL_RATE_SUPPORT_EHT
	HAL_RS_EHT_1SS		/* for all EHT */
#endif
};
...
{
	struct dvobj_priv *devob = adapter_to_dvobj(adapter);
	void *phl = GET_PHL_INFO(devob);
	u8 ratio = rtw_phl_get_tx_tbl_to_tx_pwr_times(phl);
	u8 wl_mode = rtw_hw_get_wireless_mode(devob);
	const struct phl_pwr_byrate_stbl_sel *stbls;
	struct phl_pwr_byrate_val_sel val_sel;
	struct phl_pwr_byrate_offset_sel offset_sel;
	u8 stbl_num;
	u8 stbl_idx;
	int rs, tx_num, i;
	u8 rate_num, value;

	rtw_phl_get_pwr_byrate_stbls(phl, &stbls, &stbl_num);

	for (stbl_idx = 0; stbl_idx < stbl_num; stbl_idx++) {
		val_sel.ofdma = stbls[stbl_idx].ofdma;
		val_sel.band = stbls[stbl_idx].band;
		val_sel.bw = stbls[stbl_idx].bw;

		if (!rtw_hw_is_band_support(devob, val_sel.band))
			continue;
		if (val_sel.bw != CHANNEL_WIDTH_MAX && !rtw_hw_is_bw_support(devob, val_sel.bw))
			continue;

		RTW_PRINT_SEL(sel, "[%s]", band_str(val_sel.band));
		if (val_sel.bw != CHANNEL_WIDTH_MAX)
			_RTW_PRINT_SEL(sel, "[%s]", ch_width_str(val_sel.bw));
		if (val_sel.ofdma > 0)
			_RTW_PRINT_SEL(sel, "[OFDMA]");
		_RTW_PRINT_SEL(sel, "\n");

		for (rs = 0; rs < HAL_RS_NUM; rs++) {
			tx_num = hal_rate_sec_tx_num(rs);
			if (tx_num >= GET_PHY_TX_NSS_BY_BAND(devob, HW_BAND_0))
				continue;
			if (!(wl_mode & hal_rate_sec_wl_mode(rs)))
				continue;
			if (val_sel.band != BAND_ON_24G && IS_CCK_HAL_RS(rs))
				continue;
			if (val_sel.band == BAND_ON_6G && (IS_HT_HAL_RS(rs) || IS_VHT_HAL_RS(rs)))
				continue;
			if (val_sel.ofdma == 1 && (IS_CCK_HAL_RS(rs) || IS_OFDM_HAL_RS(rs)))
				continue;

			val_sel.dcm = IS_HEDCM_HAL_RS(rs) ? 1 : 0;
			rate_num = hal_rate_sec_rate_num(rs);

			RTW_PRINT_SEL(sel, "%9s: ", hal_rate_sec_str(rs));

			/* dump power by rate in db */
			for (i = rate_num - 1; i >= 0; i--) {
				val_sel.rate = hal_rates_by_sec[rs].rates[i];
				value = rtw_phl_get_pwr_byrate_val_raw_tbl(phl, val_sel);;
				if (value % ratio) {
		...
		if (val_sel.ofdma <= 0
			&& (val_sel.bw == CHANNEL_WIDTH_20 || val_sel.bw == CHANNEL_WIDTH_MAX)
		) {
			/* dump offset */
			offset_sel.band = val_sel.band;
			RTW_PRINT_SEL(sel, "%9s: ", "Offset");
			for (i = 0; i < ARRAY_SIZE(halrs_of_byrate_offset); i++) {
				rs = halrs_of_byrate_offset[i];
				if (offset_sel.band != BAND_ON_24G && IS_CCK_HAL_RS(rs))
					continue;
				if (offset_sel.band == BAND_ON_6G && (IS_HT_HAL_RS(rs) || IS_VHT_HAL_RS(rs)))
					continue;

				offset_sel.rate = hal_rates_by_sec[rs].rates[0];
				value = rtw_phl_get_pwr_byrate_offset_raw_tbl(phl, offset_sel);
				if (value % ratio) {
