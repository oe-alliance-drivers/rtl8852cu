			...
			struct rtw_chan_def new_chan_def)
{
	...
	rtw_dfs_rd_en_dec_on_mlme_act(alink, MLME_OPCH_SWITCH, 0);
	#endif
}
...
static void rtw_ecsa_update_ap_chan_info(struct _ADAPTER *a, struct rtw_chan_def new_chan_def)
{
	...
	if (ecsa_info->bss_param) {
		...
		parm->excl_ifbmp = 0;
		parm->ch_sel.band = new_chan_def.band;
		parm->ch_sel.ch = new_chan_def.chan;
		parm->ch_sel.bw = new_chan_def.bw;
		parm->ch_sel.offset = new_chan_def.offset;
...
{
	struct _ADAPTER_LINK *iflink, *ld_iflink = NULL;
	u8 iflink_cnt = 0;
	u8 i;
	...
	for (i = 0; i < MAX_IFACE_LINK_BMP_SIZE; i++) {
		...
		if (!iflink || hwband != ALINK_GET_HWBAND(iflink))
			continue;
		if (iflink_cnt++) {
			/* multiple iflink, CSA not support */
			return NULL;
		}
		if (rtw_linked_check_alink(iflink))
			ld_iflink = iflink;
	}
