...
static u8 _to_diag_tdmra_mode(u8 tdmra_mode)
{
	switch (tdmra_mode) {
	case RTW_PHL_TDMRA_WMODE_NONE:
		return PHL_DIAG_MCC_TDMRA_MODE_NONE;
	case RTW_PHL_TDMRA_AP_CLIENT_WMODE:
		return PHL_DIAG_MCC_TDMRA_MODE_AP_CLIENT;
	case RTW_PHL_TDMRA_2CLIENTS_WMODE:
		return PHL_DIAG_MCC_TDMRA_MODE_2CLIENTS;
	case RTW_PHL_TDMRA_AP_WMODE:
		return PHL_DIAG_MCC_TDMRA_MODE_AP;
	case RTW_PHL_TDMRA_UNKNOWN_WMODE:
	default:
		return PHL_DIAG_MCC_TDMRA_MODE_UNKNOWN;
	}
}

static void _mcc_issue_diag_evt(
	struct phl_info_t *phl,
	struct phl_mcc_info *minfo,
	u8 err_type)
{
	struct rtw_phl_diag_mcc_info_v1 content = {0};

	if (!phl || !minfo)
		return;

	if (err_type != PHL_DIAG_MCC_ERR_FW_DIS_FAIL &&
		err_type != PHL_DIAG_MCC_ERR_FW_EN_FAIL)
		return;

	content.err_type = err_type;
	content.tdmra_mode = _to_diag_tdmra_mode(minfo->mcc_mode);
	phl_send_diag_hub_msg(
		phl, PHL_DIAG_EVT_MCC, 0, PHL_DIAG_LVL_CRITICAL, 1,
		(u8 *)&content, sizeof(struct rtw_phl_diag_mcc_info_v1));
}

#define _MCC_ISSUE_DIS_FAIL_EVT(_phl, _minfo) \
	_mcc_issue_diag_evt(_phl, _minfo, PHL_DIAG_MCC_ERR_FW_DIS_FAIL)
#define _MCC_ISSUE_EN_FAIL_EVT(_phl, _minfo) \
	_mcc_issue_diag_evt(_phl, _minfo, PHL_DIAG_MCC_ERR_FW_EN_FAIL)
#else
#define _MCC_ISSUE_DIS_FAIL_EVT(_phl, _minfo)
#define _MCC_ISSUE_EN_FAIL_EVT(_phl, _minfo)
#endif
			...
			struct phl_info_t *phl, struct phl_mcc_info *minfo)
{
	...
	if (_mcc_discision_duration_for_2role_bt(minfo)) {
		...
		en_info->m_pattern.bcns_offset = AP_CLIENT_OFFSET;
		if (!_mcc_fill_2wrole_pattern_with_limitation(minfo, role_ref,
								role_ano)) {
                              ...
                              struct rtw_wifi_role_link_t *cur_rlink)
{
	...
	if (rtw_hal_mcc_enable(phl->hal, en_info, &minfo->bt_info,
		...
		_MCC_ISSUE_EN_FAIL_EVT(phl, minfo);
				...
				struct rtw_wifi_role_t *spec_role)
{
	...
	if (rtw_hal_mcc_disable(phl->hal, m_role->group, m_role->macid,
		...
		_MCC_ISSUE_DIS_FAIL_EVT(phl, minfo);
