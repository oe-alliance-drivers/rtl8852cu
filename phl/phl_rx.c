				...
				struct rtw_phl_rx_pkt *phl_rx)
{
	...
	do {
		if (NULL == phl_rx) {
			PHL_TRACE(COMP_PHL_RECV, _PHL_WARNING_, "phl_rx is NULL!\n");
			break;
		}

		rx_buf = (struct rtw_rx_buf *)phl_rx->rxbuf_ptr;

		PHL_TRACE(COMP_PHL_RECV, _PHL_DEBUG_, "[4] %s:: [%p], type %d, seq 0x%03x (tid %d)\n",
		          __func__, rx_buf, phl_rx->type, phl_rx->r.mdata.seq, phl_rx->r.mdata.tid);
		if (phl_rx->rxbuf_ptr) {
		...
		if (RTW_PHL_STATUS_SUCCESS != pstatus && phl_rx->rxbuf_ptr)
			PHL_TRACE(COMP_PHL_RECV, _PHL_WARNING_, "recycle hci rx buf error!\n");
...
void phl_sta_rx_reorder_timer_expired(void *t)
{
	...
	PHL_TRACE(COMP_PHL_RECV, _PHL_INFO_, "Rx reorder timer expired, sta=0x%p\n", sta);

#ifdef CONFIG_PHL_TID_RX_DROP_INFO
	phl_dump_sta_tid_rx_drop_info(phl_info, sta);
#endif
...
#ifdef CONFIG_PHL_TID_RX_DROP_INFO
const char *_drop_sts_to_str(u8 sts)
{
	switch (sts) {
	case RX_DROP_INFO_STS_IDLE:
		return "IDLE";
	case RX_DROP_INFO_STS_DROPPING:
		return "DROPPING";
	default:
		return "unknown";
	}
...
const char *_drop_rsn_to_str(u8 rsn)
{
	switch (rsn) {
	case RX_DROP_OUT_OF_DATE:
		return "out of date";
	case RX_DROP_ALREADY_STORED:
		return "already stored";
	case RX_DROP_IV_REPLAY:
		return "iv replay";
	case RX_DROP_OUT_OF_WINDOW:
...
static void _dump_tid_rx_drop_info(struct phl_tid_rx_drop_info *drop_info,
                                   const char *dump_rsn)
{
	if (!drop_info)
		return;

	PHL_INFO("[%d][%s] Rx %s (%s): cnt %d, drop_start_seq 0x%03x, drop_recover_seq 0x%03x, duration %dms (tid %d)\n",
	         drop_info->idx, _drop_sts_to_str(drop_info->sts),
	         _drop_rsn_to_str(drop_info->rsn), dump_rsn, drop_info->cnt,
	         drop_info->drop_start_seq, drop_info->drop_recover_seq,
	         drop_info->duration, drop_info->tid);

}

#ifdef CONFIG_PHL_DIAGNOSE
#define DIAG_TID_RX_DROP_CNT_INDIC_TH 10 /* multipled by buffer_size */
#define DIAG_TID_RX_DROP_DUR_INDIC_TH 10000 /* ms */
#define DIAG_TID_RX_DROP_CNT_END_INDIC_TH 10 /* multipled by buffer_size */
#define DIAG_TID_RX_DROP_DUR_END_INDIC_TH 10000 /* ms */
static bool _check_indicate_diag_tid_rx_evt(struct phl_tid_ampdu_rx *r,
                                            struct phl_tid_rx_drop_info *drop_info)
{
	u16 indicate_drop_cnt_th = 0, indicate_drop_cnt_end_th = 0;

	if (!r || !drop_info)
		return false;

	indicate_drop_cnt_th = DIAG_TID_RX_DROP_CNT_INDIC_TH * r->buf_size;
	indicate_drop_cnt_end_th = DIAG_TID_RX_DROP_CNT_END_INDIC_TH * r->buf_size;

	if (drop_info->last_indic_duration == 0) {
		if (drop_info->cnt >= indicate_drop_cnt_th &&
		    drop_info->duration >= DIAG_TID_RX_DROP_DUR_INDIC_TH) {
			PHL_WARN("[DIAG][PHL] indicate initial diag tid rx drop event!\n");
			drop_info->last_indic_duration = drop_info->duration;
			drop_info->last_indic_drop_cnt = drop_info->cnt;
			return true;
		}
	} else {
		if (drop_info->sts == RX_DROP_INFO_STS_DROPPING) {
			if (drop_info->duration > drop_info->last_indic_duration) {
				if ((drop_info->duration - drop_info->last_indic_duration >=
				    DIAG_TID_RX_DROP_DUR_INDIC_TH) &&
				    (drop_info->cnt - drop_info->last_indic_drop_cnt >=
				    indicate_drop_cnt_th)) {
				    PHL_WARN("[DIAG][PHL] indicate continous diag tid rx drop event!\n");
				    drop_info->last_indic_duration = drop_info->duration;
				    drop_info->last_indic_drop_cnt = drop_info->cnt;
				    return true;
				}
			...
			if (drop_info->duration >= DIAG_TID_RX_DROP_DUR_END_INDIC_TH &&
			    drop_info->cnt >= indicate_drop_cnt_end_th) {
			    PHL_WARN("[DIAG][PHL] indicate end diag tid rx drop event!\n");
			    drop_info->last_indic_duration = drop_info->duration;
			    drop_info->last_indic_drop_cnt = drop_info->cnt;
			    return true;
			}
...
static void _indicate_diag_tid_rx_evt(struct phl_tid_ampdu_rx *r,
                                      struct phl_tid_rx_drop_info *drop_info)
{
	struct rtw_phl_diag_tid_rx_info_v1 diag_tid_rx_info = {0};

	if (!r || !drop_info)
		return;

	if (drop_info->rsn == RX_DROP_OUT_OF_DATE)
		diag_tid_rx_info.evt = PHL_DIAG_TID_RX_DROP_OUT_OF_DATE;
	else if (drop_info->rsn == RX_DROP_ALREADY_STORED)
		diag_tid_rx_info.evt = PHL_DIAG_TID_RX_DROP_ALREADY_STORED;
	else if (drop_info->rsn == RX_DROP_IV_REPLAY)
		diag_tid_rx_info.evt = PHL_DIAG_TID_RX_DROP_IV_REPLAY;
	else if (drop_info->rsn == RX_DROP_OUT_OF_WINDOW)
		diag_tid_rx_info.evt = PHL_DIAG_TID_RX_DROP_OUT_OF_WINDOW;
	else
		return;

	_os_mem_cpy(r->drv_priv, diag_tid_rx_info.mac_addr, r->sta->mac_addr, MAC_ALEN);
	diag_tid_rx_info.tid = drop_info->tid;
	diag_tid_rx_info.cnt = drop_info->cnt;
	diag_tid_rx_info.duration = drop_info->duration;
	diag_tid_rx_info.drop_start_seq = drop_info->drop_start_seq;
	diag_tid_rx_info.drop_recover_seq = drop_info->drop_recover_seq;
	diag_tid_rx_info.current_head_seq = r->head_seq_num;

	phl_send_diag_hub_msg(r->phl_info, PHL_DIAG_EVT_TID_RX,
	                      INVALID_SUBMODULE_DIAG_EVT, PHL_DIAG_LVL_CRITICAL, 1,
	                      (u8 *)&diag_tid_rx_info, sizeof(struct rtw_phl_diag_tid_rx_info_v1));
}
#endif /* CONFIG_PHL_DIAGNOSE */

static void _dump_tid_ampdu_rx_drop_info(struct phl_tid_ampdu_rx *r)
{
	u8 rsn = 0, idx = 0;
	...
	if (!r->started || r->sleep || r->removed)
		return;

	for (rsn = RX_DROP_RSN_MIN; rsn < RX_DROP_RSN_MAX; rsn++) {
		idx = r->drop_info_idx[rsn];
		if (r->drop_info[rsn][idx].sts == RX_DROP_INFO_STS_DROPPING) {
			_dump_tid_rx_drop_info(&r->drop_info[rsn][idx], "check dump");
		}
	}
}

static void _dump_sta_tid_rx_drop_info(struct phl_info_t *phl_info, struct rtw_phl_stainfo_t *sta)
{
	void *drv = phl_to_drvpriv(phl_info);
	u8 i = 0;
	...
	if (sta) {
		for (i = 0; i < ARRAY_SIZE(sta->tid_rx); i++) {
			...
			_dump_tid_ampdu_rx_drop_info(sta->tid_rx[i]);
			_os_spinunlock(drv, &sta->tid_rx_lock, _bh, NULL);
		}
	}
}

static void _record_tid_rx_drop(struct phl_tid_ampdu_rx *r, u16 seq, u8 rsn)
{
	struct phl_tid_rx_drop_info *drop_info = NULL;
	u8 idx = 0;
	...
	idx = r->drop_info_idx[rsn];
	drop_info = r->drop_info[rsn];

	/* start a new record */
	if (drop_info[idx].sts == RX_DROP_INFO_STS_IDLE) {
		drop_info[idx].cnt = 0;
		drop_info[idx].sts = RX_DROP_INFO_STS_DROPPING;
		drop_info[idx].start_time = _os_get_cur_time_ms();
		drop_info[idx].drop_start_seq = seq;
		drop_info[idx].drop_recover_seq = 0xFFFF;
		drop_info[idx].tid = r->tid;
#ifdef CONFIG_PHL_DIAGNOSE
		drop_info[idx].last_indic_duration = 0;
		drop_info[idx].last_indic_drop_cnt = 0;
#endif;
	}

	/* record status */
	if (drop_info[idx].cnt != 0xFFFF)
		drop_info[idx].cnt++;
	drop_info[idx].duration = phl_get_passing_time_ms(drop_info[idx].start_time);

#ifdef CONFIG_PHL_DIAGNOSE
	if (_check_indicate_diag_tid_rx_evt(r, &drop_info[idx])) {
		_indicate_diag_tid_rx_evt(r, &drop_info[idx]);
		_dump_tid_rx_drop_info(&drop_info[idx], "diag indicate");
	};
#endif
	/*_dump_tid_ampdu_rx_drop_info(r);*/
}

static void _end_record_tid_rx_drop(struct phl_tid_ampdu_rx *r, u16 seq)
{
	struct phl_tid_rx_drop_info *drop_info = NULL;
	u8 *idx = NULL;
	u8 rsn = 0;
	...
	for (rsn = RX_DROP_RSN_MIN; rsn < RX_DROP_RSN_MAX; rsn++) {
		...
		if (drop_info[*idx].sts == RX_DROP_INFO_STS_DROPPING) {
			drop_info[*idx].sts = RX_DROP_INFO_STS_IDLE;
			drop_info[*idx].drop_recover_seq = seq;
			drop_info[*idx].duration = phl_get_passing_time_ms(drop_info[*idx].start_time);
			_dump_tid_rx_drop_info(&drop_info[*idx], "drop end");
#ifdef CONFIG_PHL_DIAGNOSE
			if (_check_indicate_diag_tid_rx_evt(r, &drop_info[*idx])) {
				_indicate_diag_tid_rx_evt(r, &drop_info[*idx]);
				_dump_tid_rx_drop_info(&drop_info[*idx], "diag indicate");
...
#endif
			*idx = (*idx + 1) % MAX_PHL_RX_DROP_INFO_RECORED;
		}
	}
}

void phl_dump_sta_tid_rx_drop_info(struct phl_info_t *phl_info,
                                   struct rtw_phl_stainfo_t *target_sta)
{
	...
	drv = phl_to_drvpriv(phl_info);

	if (target_sta) {
		_dump_sta_tid_rx_drop_info(phl_info, target_sta);
	} else { /* dump all */
		for (i = 0; i < MAX_WIFI_ROLE_NUMBER; i++) {
			role = &phl_com->wifi_roles[i];
			if (role->active && (role->mstate == MLME_LINKED)) {
				for (ridx = 0; ridx < role->rlink_num; ridx++) {
					rlink = get_rlink(role, ridx);
					...
					phl_list_for_loop(sta, struct rtw_phl_stainfo_t,
						...
						if (sta)
							_dump_sta_tid_rx_drop_info(phl_info, sta);
					}
...
#endif /* CONFIG_PHL_TID_RX_DROP_INFO */
                                       ...
                                       _os_list *frames)
{
	...
	int index;
	struct phl_hci_trx_ops *hci_trx_ops = phl_info->hci_trx_ops;
#ifdef CONFIG_PHL_TID_RX_IV_CHK
...
#ifdef CONFIG_PHL_TID_RX_DROP_INFO
		_record_tid_rx_drop(r, meta->seq, RX_DROP_OUT_OF_DATE);
#endif
...
#ifdef CONFIG_PHL_TID_RX_IV_CHK
	if (evt_ops->sec_check_iv_valid &&
	    !evt_ops->sec_check_iv_valid(phl_to_drvpriv(phl_info), &pkt->r)) {
		PHL_TRACE(COMP_PHL_RECV, _PHL_WARNING_, "Rx drop (iv replay): old seq 0x%03x, head 0x%03x (tid=%u)\n",
		          meta->seq, r->head_seq_num, r->tid);
#ifdef CONFIG_PHL_TID_RX_DROP_INFO
		_record_tid_rx_drop(r, meta->seq, RX_DROP_IV_REPLAY);
#endif
		hci_trx_ops->recycle_rx_pkt(phl_info, pkt);
		return true;
	}
	...
	if (!seq_less(mpdu_seq_num, head_seq_num + buf_size)) {
		if (seq_less(mpdu_seq_num, head_seq_num + (SEQ_MODULO >> 1))) {
			...
			PHL_TRACE(COMP_PHL_RECV, _PHL_WARNING_, "Rx drop (out of window): seq 0x%03x, head 0x%03x (tid=%u)\n",
			          meta->seq, r->head_seq_num, r->tid);
#ifdef CONFIG_PHL_TID_RX_DROP_INFO
			_record_tid_rx_drop(r, meta->seq, RX_DROP_OUT_OF_WINDOW);
#endif
			hci_trx_ops->recycle_rx_pkt(phl_info, pkt);
			return true;
		}
	...
	if (r->reorder_buf[index]) {
		PHL_TRACE(COMP_PHL_RECV, _PHL_DEBUG_, "Rx drop (already stored): old seq 0x%03x, head 0x%03x (tid %d)\n",
				meta->seq, r->head_seq_num, r->tid);
#ifdef CONFIG_PHL_TID_RX_DROP_INFO
		_record_tid_rx_drop(r, meta->seq, RX_DROP_ALREADY_STORED);
#endif
		hci_trx_ops->recycle_rx_pkt(phl_info, pkt);
#ifdef DEBUG_PHL_RX
		phl_info->rx_stats.rx_drop_reorder++;
...
#ifdef CONFIG_PHL_TID_RX_DROP_INFO
	_end_record_tid_rx_drop(r, meta->seq);
#endif
	/*
...
#ifdef CONFIG_PHL_TID_RX_DROP_INFO
	phl_dump_sta_tid_rx_drop_info(phl_info, NULL);
#endif
