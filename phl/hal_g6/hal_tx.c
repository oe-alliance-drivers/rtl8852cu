...
/**
 * rtw_hal_get_wd_len - get wd len for xmit packet
 * @hal: see struct hal_info_t
 * @tx_req: packet xmit request from phl, see struct rtw_phl_pkt_req
 *
 * returns wd_len
 */
				...
				struct rtw_xmit_req *tx_req)
{
	...
	if (trx_ops->hal_get_wd_len)
		return trx_ops->hal_get_wd_len(hal_info, tx_req);
	return rtw_hal_mac_get_txdesc_len(hal_info->mac, tx_req);
}
