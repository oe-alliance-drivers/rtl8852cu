...
rtw_hal_mac_read_hidden_rpt(struct rtw_hal_com_t *hal_com)
{
	...
	if (err != MACSUCCESS) {
		PHL_TRACE(COMP_PHL_MAC, _PHL_INFO_, "%s fail, err=%d\n", __func__, err);
		return RTW_HAL_STATUS_FAILURE;
	}
...
enum rtw_hal_status
rtw_hal_mac_ips_chk_leave(struct hal_info_t *hal_info, u16 macid, u32 *mac_sts)
{
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);

	*mac_sts = mac->ops->chk_leave_ips(mac, (u8)macid);
			...
			struct rtw_hal_fw_log_cfg *fl_cfg)
{
	...
	if (log_cfg.output == MAC_AX_FL_LV_UART) {
		status = mac->ops->get_uart_fw_dbg_gpio(mac, &uart_tx_pin, &uart_rx_pin);
		if (status != MACSUCCESS) {
			PHL_ERR ("get_uart_fw_dbg_gpio fail %d\n", status);
			return RTW_HAL_STATUS_FAILURE;
		}

		status = mac->ops->set_gpio_func(mac, RTW_MAC_GPIO_WL_UART_TX, uart_tx_pin);
		if (status != MACSUCCESS) {
			PHL_ERR ("set_gpio_func UART TX fail %d %d\n", status, uart_tx_pin);
			return RTW_HAL_STATUS_FAILURE;
		}

		status = mac->ops->set_gpio_func(mac, RTW_MAC_GPIO_WL_UART_RX, uart_rx_pin);
		if (status != MACSUCCESS) {
			PHL_ERR ("set_gpio_func UART RX fail %d %d\n", status, uart_rx_pin);
			return RTW_HAL_STATUS_FAILURE;
		}
	}
...
static const struct mac_mem_sel mac_mem_tbl[] = {
	{MAC_AX_MEM_DMAC_TBL, sizeof(struct mac_ax_dctl_info),
	 "dmac_tbl", "DMAC Table"},
	{MAC_AX_MEM_SHCUT_MACHDR, sizeof(struct mac_ax_shcut_mhdr),
	 "shcut_machdr", "TX Shortcut WHDR"},
	{MAC_AX_MEM_SECURITY_CAM, 0x20,
	 "sec_cam", "Security CAM"},
	{MAC_AX_MEM_CMAC_TBL, sizeof(struct mac_ax_cctl_info),
	 "cmac_tbl", "CMAC Table"},
	{MAC_AX_MEM_ADDR_CAM, 0x40,
	 "addr_cam", "Address CAM"},
	{MAC_AX_MEM_BA_CAM, 0x64,
	 "ba_cam", "BA CAM"},
};

void rtw_hal_mac_dump_mac_mem(struct hal_info_t *hal,
                              char *cmd,
                              u32 idx,
                              u8 *buf)
{
	...
	u8 i;
	u8 _buf[256];
	u16 entry_size = sizeof(_buf);
	u32 status;

	for (i = 0; i < sizeof(mac_mem_tbl)/sizeof(mac_mem_tbl[0]); i++) {
		if (_os_strcmp(cmd, mac_mem_tbl[i].cmd) == 0) {
			if (mac_mem_tbl[i].mem_sel == MAC_AX_MEM_ADDR_CAM)
				entry_size = get_addr_cam_size(mac);
			else if (mac_mem_tbl[i].entry_size < entry_size)
				entry_size = mac_mem_tbl[i].entry_size;
			break;
		}
	}

	if (i >= sizeof(mac_mem_tbl)/sizeof(mac_mem_tbl[0])) {
		PHL_WARN("%s: invalid CMD %s\n", __func__, cmd);
		return;
	}

	status = mac_mem_dump(mac,
	                      mac_mem_tbl[i].mem_sel,
	                      idx * entry_size,
	                      _buf,
	                      entry_size,
	                      1);
	if (status != MACSUCCESS) {
		PHL_WARN("Failed to dump %s of index %d with reason %d.\n",
		         mac_mem_tbl[i].name, idx, status);
		return;
	}

	if (buf != NULL) {
		_os_mem_cpy(hal->hal_com->drv_priv, buf, _buf, entry_size);
	} else {
		PHL_PRINT("Dumping %s of index %u...\n",
		          mac_mem_tbl[i].name, idx);
		debug_dump_buf(_buf, entry_size, mac_mem_tbl[i].name);
	}
}
...
enum rtw_hal_status
rtw_hal_mac_nan_join_cluster(struct hal_info_t *hal, u8 is_allow)
{
	struct mac_ax_adapter *mac = hal_to_mac(hal);
	u32 m_stat = 0;

	if(!mac->ops->nan_join_cluster)
		return RTW_HAL_STATUS_FAILURE;

	m_stat = mac->ops->nan_join_cluster(mac, is_allow);

	return (m_stat != MACSUCCESS) ?
		RTW_HAL_STATUS_FAILURE : RTW_HAL_STATUS_SUCCESS;
}
			...
			u16 tx_interval)
{
	...
	return hal_status;
#else /* !CONFIG_PHL_THERMAL_PROTECT */
	PHL_WARN("%s: CONFIG_PHL_THERMAL_PROTECT not support!\n", __func__);
