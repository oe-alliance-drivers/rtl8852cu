...
{
	u16 max_agg_num, ampdu_num_limit;
	u8 band;
	u32 ret;
	...
	ampdu_num_limit = band ? adapter->hw_info->max_tx_ampdu_num :
			  adapter->hw_info->max_tx_ampdu_num_c1;
	...
	if (max_agg_num > 0 && max_agg_num <= ampdu_num_limit) {
		val32 = SET_CLR_WORD(val32, max_agg_num - 1, B_AX_MAX_AGG_NUM);
	} else {
		val32 = SET_CLR_WORD(val32, ampdu_num_limit - 1, B_AX_MAX_AGG_NUM);
		PLTFM_MSG_ERR("%s invalid max_agg_num %d, corrected to %d, band %d",
			      __func__, max_agg_num, ampdu_num_limit, band);
	}
	if (max_agg_time > 0 && max_agg_time <= txtime_limit) {
		...
		PLTFM_MSG_ERR("%s invalid max_agg_time 0x%x, corrected to 0x%x",
			      __func__, max_agg_time, txtime_limit);
	}
