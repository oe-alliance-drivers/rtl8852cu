...
static const struct mac_hal_cmd_info mac_hal_cmd_i[] = {
	...
	{"wicense_dbg", MAC_HAL_WICENSE_DBG, cmd_mac_wicense_dbg, "WiCense debug"},
			   ...
			   u32 input_num, char *output, u32 out_len, u32 *used)
{
	...
	if (diag_level <= MAC_DIAG_LEVEL_3) {
		//System Status check
		ret = mac_sys_st_check(adapter, diag_level, &check_number, output, out_len, used);
		MAC_DBG_MSG(out_len, *used, output + *used, out_len - *used,
...
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 dma_idx, cpu_idx, used_pg, aval_pg, i, addr, pg0, pg1, val32;

	PLTFM_MSG_ALWAYS("TX Related Infomation\n");
	if (is_chip_id(adapter, MAC_AX_CHIP_ID_8852A) ||
	    is_chip_id(adapter, MAC_AX_CHIP_ID_8852B) ||
	    is_chip_id(adapter, MAC_AX_CHIP_ID_8851B) ||
	    is_chip_id(adapter, MAC_AX_CHIP_ID_8852BT)) {
#if MAC_AX_8852A_SUPPORT || MAC_AX_8852B_SUPPORT || MAC_AX_8851B_SUPPORT || MAC_AX_8852BT_SUPPORT
		for (i = MAC_AX_DMA_ACH0; i <= MAC_AX_DMA_B1HI; i++) {
			if (IS_TXDMA_CH_VALID(i)) {
				addr = R_AX_ACH0_TXBD_IDX + i * 4;
				val32 = MAC_REG_R32(addr);
				dma_idx = GET_FIELD(val32, B_AX_ACH0_HW_IDX);
				cpu_idx = GET_FIELD(val32, B_AX_ACH0_HOST_IDX);
				addr = R_AX_ACH0_PAGE_INFO + i * 4;
				val32 = MAC_REG_R32(addr);
				aval_pg = GET_FIELD(val32, B_AX_ACH0_AVAL_PG);
				used_pg = GET_FIELD(val32, B_AX_ACH0_USE_PG);
				PLTFM_MSG_ALWAYS("CH%d_TXBD IDX(DMA/CPU) = 0x%04x/0x%04x " \
						 "PAGE(Aval/Used) = 0x%04x/0x%04x\n",
						 i, dma_idx, cpu_idx, aval_pg, used_pg);
			} else {
				PLTFM_MSG_ALWAYS("CH%d Invalid\n", i);
			}
		}
		val32 = MAC_REG_R32(R_AX_PUB_PAGE_INFO1);
		pg0 = GET_FIELD(val32, B_AX_G0_USE_PG);
		pg1 = GET_FIELD(val32, B_AX_G1_USE_PG);
		PLTFM_MSG_ALWAYS("Used page for group 0/1: 0x%04x/0x%04x\n", pg0, pg1);
		val32 = MAC_REG_R32(R_AX_PUB_PAGE_INFO3);
		pg0 = GET_FIELD(val32, B_AX_G0_AVAL_PG);
		pg1 = GET_FIELD(val32, B_AX_G1_AVAL_PG);
		PLTFM_MSG_ALWAYS("Aval page for group 0/1: 0x%04x/0x%04x\n", pg0, pg1);
		aval_pg = GET_FIELD(MAC_REG_R32(R_AX_PUB_PAGE_INFO2), B_AX_PUB_AVAL_PG);
		PLTFM_MSG_ALWAYS("Aval page for public: 0x%04x\n", aval_pg);
		aval_pg = GET_FIELD(MAC_REG_R32(R_AX_WP_PAGE_INFO1), B_AX_WP_AVAL_PG);
		PLTFM_MSG_ALWAYS("Aval page for WP: 0x%04x\n", aval_pg);

		PLTFM_MSG_ALWAYS("RX Related Infomation\n");
		if (IS_RXDMA_CH_VALID(MAC_BE_RX_CH_RXQ)) {
			val32 = MAC_REG_R32(R_AX_RXQ_RXBD_IDX);
			dma_idx = GET_FIELD(val32, B_AX_RXQ_HW_IDX);
			cpu_idx = GET_FIELD(val32, B_AX_RXQ_HOST_IDX);
			PLTFM_MSG_ALWAYS("RXQ0 BD IDX(DMA/CPU): 0x%04x/0x%04x\n", dma_idx, cpu_idx);
		} else {
			PLTFM_MSG_ALWAYS("RXQ0 Invalid\n");
		}
		if (IS_RXDMA_CH_VALID(MAC_BE_RX_CH_RPQ)) {
			val32 = MAC_REG_R32(R_AX_RPQ_RXBD_IDX);
			dma_idx = GET_FIELD(val32, B_AX_RPQ_HW_IDX);
			cpu_idx = GET_FIELD(val32, B_AX_RPQ_HOST_IDX);
			PLTFM_MSG_ALWAYS("RPQ0 BD IDX(DMA/CPU): 0x%04x/0x%04x\n", dma_idx, cpu_idx);
		} else {
			PLTFM_MSG_ALWAYS("RPQ0 Invalid\n");
		}
#endif
		...
		for (i = MAC_AX_DMA_ACH0; i <= MAC_AX_DMA_B1HI; i++) {
			if (IS_TXDMA_CH_VALID(i)) {
				...
				dma_idx = GET_FIELD(val32, B_AX_ACH0_HW_IDX);
				cpu_idx = GET_FIELD(val32, B_AX_ACH0_HOST_IDX);
				addr = R_AX_ACH0_PAGE_INFO_V1 + i * 4;
				val32 = MAC_REG_R32(addr);
				aval_pg = GET_FIELD(val32, B_AX_ACH0_AVAL_PG);
				used_pg = GET_FIELD(val32, B_AX_ACH0_USE_PG);
				PLTFM_MSG_ALWAYS("CH%d_TXBD IDX(DMA/CPU) = 0x%04x/0x%04x " \
		...
		pg1 = GET_FIELD(val32, B_AX_G1_AVAL_PG);
		PLTFM_MSG_ALWAYS("Aval page for group 0/1: 0x%04x/0x%04x\n", pg0, pg1);
		aval_pg = GET_FIELD(MAC_REG_R32(R_AX_PUB_PAGE_INFO2_V1), B_AX_PUB_AVAL_PG);
		PLTFM_MSG_ALWAYS("Aval page for public: 0x%04x\n", aval_pg);
		aval_pg = GET_FIELD(MAC_REG_R32(R_AX_WP_PAGE_INFO1_V1), B_AX_WP_AVAL_PG);
		PLTFM_MSG_ALWAYS("Aval page for WP: 0x%04x\n", aval_pg);

		PLTFM_MSG_ALWAYS("RX Related Infomation\n");
		if (IS_RXDMA_CH_VALID(MAC_BE_RX_CH_RXQ)) {
			val32 = MAC_REG_R32(R_AX_RXQ_RXBD_IDX_V1);
			dma_idx = GET_FIELD(val32, B_AX_RXQ_HW_IDX);
			cpu_idx = GET_FIELD(val32, B_AX_RXQ_HOST_IDX);
			PLTFM_MSG_ALWAYS("RXQ0 BD IDX(DMA/CPU): 0x%04x/0x%04x\n", dma_idx, cpu_idx);
		} else {
		...
		if (IS_RXDMA_CH_VALID(MAC_BE_RX_CH_RPQ)) {
			val32 = MAC_REG_R32(R_AX_RPQ_RXBD_IDX_V1);
			dma_idx = GET_FIELD(val32, B_AX_RPQ_HW_IDX);
			cpu_idx = GET_FIELD(val32, B_AX_RPQ_HOST_IDX);
			PLTFM_MSG_ALWAYS("RPQ0 BD IDX(DMA/CPU): 0x%04x/0x%04x\n", dma_idx, cpu_idx);
		} else {
...
u32 cmd_mac_wicense_dbg(struct mac_ax_adapter *adapter, char input[][MAC_MAX_ARGV], u32 input_num,
		     char *output, u32 out_len, u32 *used)
{
	...
	if (csi_info->dbg_rpt_mode == WICENSE_DBG_RPT_MODE_DIS) {
		MAC_DBG_MSG(out_len, *used, output + *used, out_len - *used,
			    "=> dbg_rpt_mode is disable!!\n");
		return MACFUNCINPUT;
	}
	...
	MAC_DBG_MSG(out_len, *used, output + *used, out_len - *used,
		    "[WisenseRpt] ===> Summary report start....\n");
	MAC_DBG_MSG(out_len, *used, output + *used, out_len - *used,
		    "[WisenseRpt] error state: %x\n",
		    rpt_all->error_state);
	MAC_DBG_MSG(out_len, *used, output + *used, out_len - *used,
		    "[WisenseRpt] cnt (enable/disable/pause/resume): %d/%d/%d/%d\n",
		    rpt_all->cnt_wicense_enable,
		    rpt_all->cnt_wicense_disable,
		    rpt_all->cnt_wicense_pause,
		    rpt_all->cnt_wicense_resume);
	MAC_DBG_MSG(out_len, *used, output + *used, out_len - *used,
		    "[WisenseRpt] txdata cnt (send ok/send fail/tx ok/tx fail/Tx Fail Rate): %d/%d/%d/%d/%d\n",
		    ...
		    txdata_fail_rate);
	MAC_DBG_MSG(out_len, *used, output + *used, out_len - *used,
		    "[WisenseRpt] txdata fail ret cnt (rty/life/macid/invalid rpt): %d/%d/%d/%d\n",
	...
	MAC_DBG_MSG(out_len, *used, output + *used, out_len - *used,
		    "[WisenseRpt] swch fail cnt (rx0 idle/rx1 idle/txen): %d/%d/%d\n",
		    rpt_all->cnt_swch_rx0_idle_poll_fail,
		    rpt_all->cnt_swch_rx1_idle_poll_fail,
		    rpt_all->cnt_swch_txen_poll_fail);
	MAC_DBG_MSG(out_len, *used, output + *used, out_len - *used,
		    "[WisenseRpt] swch fail cnt : ctrl_bw_ch (n0/n1/s0/s1): %d/%d/%d/%d\n",
	...
	MAC_DBG_MSG(out_len, *used, output + *used, out_len - *used,
		    "[WisenseRpt] en_dis / wicense active (ms): %d/%d\n",
		    rpt_all->en_dis_time_ms,
		    rpt_all->avtive_time_ms);
	MAC_DBG_MSG(out_len, *used, output + *used, out_len - *used,
		    "[WisenseRpt] wicense_dur / sw_sensing_ch / sw_normal_ch (us): %d/%d/%d\n",
		    rpt_all->time_wicense_dur,
		    rpt_all->time_sw_sensing_ch,
		    rpt_all->time_sw_normal_ch);
	MAC_DBG_MSG(out_len, *used, output + *used, out_len - *used,
		    "[WisenseRpt] s1/s2/s3/s4/s5 dur (us): %d/%d/%d/%d/%d\n",
		    rpt_all->time_s1_s2,
		    rpt_all->time_s2_s3,
		    rpt_all->time_s3_s4,
		    rpt_all->time_s4_s5,
