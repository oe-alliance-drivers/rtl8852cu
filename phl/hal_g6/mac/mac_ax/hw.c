		     ...
		     enum mac_ax_hw_id hw_id, void *val)
{
	...
	case MAC_AX_HW_SET_HOST_RPR:
		ret = set_host_rpr(adapter, (struct mac_ax_host_rpr_cfg *)val);
		break;
	...
	case MAC_AX_HW_SET_POLLUTED:
		ret = mac_cfg_plt(adapter, (struct mac_ax_plt *)val);
		break;
	case MAC_AX_HW_SET_COEX_CTRL:
		ret = p_ops->cfg_ctrl_path(adapter, *(u32 *)val);
		break;
#endif /* MAC_FEAT_COEX */
	...
	case MAC_AX_HW_SET_SLOT_TIME:
		ret = mac_set_slot_time(adapter, *(enum mac_ax_slot_time *)val);
		break;
	...
	case MAC_AX_HW_SDIO_MON_WT:
		ret = p_ops->set_wt_cfg_sdio(adapter, *(u8 *)val);
		break;
#endif
#if MAC_AX_PCIE_SUPPORT
	...
	case MAC_AX_HW_SET_CCTL_PRELD:
		ret = set_cctl_preld(adapter, (struct mac_ax_cctl_preld_cfg *)val);
		break;
	...
	case MAC_AX_HW_SET_USB_UPHY_PLL_CFG:
		ret = usb_uphy_pll_cfg(adapter, *(u8 *)val);
		break;
		     ...
		     enum mac_ax_hw_id hw_id, void *val)
{
	...
	case MAC_AX_HW_GET_PWR_STATE:
		ret = get_pwr_state(adapter, (enum mac_ax_mac_pwr_st *)val);
		break;
#if MAC_FEAT_COEX
	...
	case MAC_AX_HW_GET_COEX_CTRL:
		ret = p_ops->get_ctrl_path(adapter, (u32 *)val);
		break;
#endif /* MAC_FEAT_COEX */
	case MAC_AX_HW_GET_BACAM_MODE_SEL:
		ret = get_bacam_mode(adapter, (u8 *)val);
		break;
	...
	case MAC_AX_HW_GET_DATA_RTY_LMT:
		ret = get_data_rty_limit(adapter, (struct mac_ax_rty_lmt *)val);
		break;
...
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	struct mac_ax_host_rpr_cfg *def_cfg = &rpr_cfg_stf;
	enum mac_ax_host_rpr_mode mode = MAC_AX_RPR_MODE_STF;
#if MAC_AX_PCIE_SUPPORT
	u8 is_poh;
#endif
	u32 val32, nval32;
 ...
 */
	...
	if (is_chip_id(adapter, MAC_AX_CHIP_ID_8852B) ||
	    is_chip_id(adapter, MAC_AX_CHIP_ID_8851B) ||
	    is_chip_id(adapter, MAC_AX_CHIP_ID_8852BT)) {
		if (wdt_param->drv_ctrl.autok_wdt_ctrl == MAC_AX_PCIE_ENABLE ||
		...
		if (wdt_param->drv_ctrl.tp_wdt_ctrl == MAC_AX_PCIE_ENABLE ||
