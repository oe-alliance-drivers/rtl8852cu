...
#endif
	{c2h_wicense_rpt_info, FWCMD_C2H_FUNC_WISENSE_DBG_RPT},
	{NULL, FWCMD_C2H_FUNC_NULL},
};
				   ...
				   struct rtw_c2h_info *info)
{
	...
	case FWCMD_H2C_FUNC_WIFI_SENSING_CSI:
		PLTFM_MSG_ALWAYS("FWCMD_H2C_FUNC_WIFI_SENSING_CSI start_cmd_send = %d, stop_cmd_send = %d\n", csi_info->start_cmd_send, csi_info->stop_cmd_send);
		...
		if (csi_info->start_cmd_send) {
			if (info->h2c_return == MACSUCCESS)
				...
				PLTFM_MSG_ERR("[start_cmd_send] info->h2c_return != MACSUCCESS\n");

			csi_info->start_cmd_send = 0;
			PLTFM_MSG_ALWAYS("set csi_info->start_cmd_send = 0, start_cmd_send = %d\n", csi_info->start_cmd_send);
		}
		...
		if (csi_info->stop_cmd_send) {
			if (info->h2c_return == MACSUCCESS)
				...
				PLTFM_MSG_ERR("[stop_cmd_send] info->h2c_return != MACSUCCESS\n");

			csi_info->stop_cmd_send = 0;
			PLTFM_MSG_ALWAYS("set csi_info->stop_cmd_send = 0, stop_cmd_send = %d\n", csi_info->stop_cmd_send);
		}

		//Remove mutex lock since it's guaranteed to ATOMIC
		sm->sensing_csi_st = MAC_AX_SENSING_CSI_IDLE;
		PLTFM_MSG_ALWAYS("%s FWCMD_H2C_FUNC_WIFI_SENSING_CSI END--------------------------------\n", __func__);
		break;
...
#endif
#if MAC_FEAT_LPS
	case FWCMD_H2C_FUNC_LPS_PARM:
	case FWCMD_H2C_FUNC_IPS_CFG:
		PLTFM_MSG_ALWAYS("[PS] DACK %d\n", info->h2c_return);
		if (info->h2c_return == MACSUCCESS)
			sm->ps_state = MAC_AX_PS_STS_ACTIVE;
		break;
				...
				u32 len, struct rtw_c2h_info *info)
{
	...
	case PKT_OFLD_OP_READ:
		if (pkt_len != 0) {
			...
			if (pkt_len > ofld_pkt->malloc_len) {
				PLTFM_MSG_ERR("[pktofld] malloc_len(%d) < pkt_len(%d)," \
					      "content is cropped\n", ofld_pkt->malloc_len,
					      pkt_len);
				pkt_len = ofld_pkt->malloc_len;
			}
			PLTFM_MEMCPY(ofld_pkt->pkt, pkt_content, pkt_len);
			ofld_pkt->pkt_id = id;
...
{
	struct sensing_csi_info *csi_info = &adapter->csi_info;
	struct rtw_hal_mac_wicense_rpt *rpt;
	struct rtw_hal_mac_wicense_rpt_all *rpt_all;
	u32 en_dis_time_ms, active_time_ms;
	u32 txdata_fail_rate;
	struct rtw_hal_mac_wicense_rpt *pkg;
	pkg = (struct rtw_hal_mac_wicense_rpt *)(buf + FWCMD_HDR_LEN);

	if (!buf) {
		PLTFM_MSG_ERR("[ERR]wicense pt no buf\n");
		return MACNPTR;
	}

	if ((len - FWCMD_HDR_LEN) != sizeof(struct rtw_hal_mac_wicense_rpt))
		PLTFM_MSG_WARN("[WARN]wicense rpt size not match! fw:%d, halmac:%d\n", len,
			       sizeof(struct rtw_hal_mac_wicense_rpt));

	rpt = &adapter->csi_info.rpt;
	rpt_all = &adapter->csi_info.rpt_all;
	...
	if (csi_info->dbg_rpt_mode == WICENSE_DBG_RPT_MODE_EN_DETAIL) {
		...
		PLTFM_MSG_ALWAYS("[WisenseRpt] macid: %d, seqnum = %d\n", rpt->macid, rpt->seq_num);
		PLTFM_MSG_ALWAYS("[WisenseRpt] error state: 0x%x\n", rpt->error_state);
		PLTFM_MSG_ALWAYS("[WisenseRpt] cnt (enable/pause/resume/disable): %d/%d/%d/%d\n",
				 rpt->cnt_wicense_enable,
				 rpt->cnt_wicense_pause,
				 rpt->cnt_wicense_resume,
				 rpt->cnt_wicense_disable);
		PLTFM_MSG_ALWAYS("[WisenseRpt] txdata cnt (send ok/send fail/tx ok/tx fail): %d/%d/%d/%d\n",
				 rpt->cnt_txdata_send_ok,
				 rpt->cnt_txdata_send_fail,
				 rpt->cnt_txdata_tx_ok,
				 rpt->cnt_txdata_tx_fail);
		PLTFM_MSG_ALWAYS("[WisenseRpt] txdata fail ret cnt (rty/life/macid/invalid rpt): %d/%d/%d/%d\n",
				 rpt->cnt_txdata_fail_rtydrop,
				 rpt->cnt_txdata_fail_lifedrop,
				 rpt->cnt_txdata_fail_maciddrop,
				 rpt->cnt_txdata_fail_invalidrpt);
		PLTFM_MSG_ALWAYS("[WisenseRpt] txnull_1 cnt (send ok/send fail/tx ok/tx fail): %d/%d/%d/%d\n",
				 rpt->cnt_txnull_1_send_ok,
				 rpt->cnt_txnull_1_send_fail,
				 rpt->cnt_txnull_1_tx_ok,
				 rpt->cnt_txnull_1_tx_fail);
		PLTFM_MSG_ALWAYS("[WisenseRpt] txnull_0 cnt (send ok/send fail/tx ok/tx fail): %d/%d/%d/%d\n",
				 rpt->cnt_txnull_0_send_ok,
				 rpt->cnt_txnull_0_send_fail,
				 rpt->cnt_txnull_0_tx_ok,
				 rpt->cnt_txnull_0_tx_fail);
		PLTFM_MSG_ALWAYS("[WisenseRpt] swch fail cnt (ser stop/ser start/rflck to/force_ch): %d/%d/%d/%d\n",
				 rpt->cnt_swch_ser_stop_fail,
				 rpt->cnt_swch_ser_start_fail,
				 rpt->cnt_swch_rflck_poll_timeout,
				 rpt->cnt_swch_force_sensing_dbg);
		PLTFM_MSG_ALWAYS("[WisenseRpt] swch fail cnt (rx0 idle/rx1 idle/txen): %d/%d/%d\n",
				 rpt->cnt_swch_rx0_idle_poll_fail,
				 rpt->cnt_swch_rx1_idle_poll_fail,
				 rpt->cnt_swch_txen_poll_fail);
		PLTFM_MSG_ALWAYS("[WisenseRpt] swch fail cnt :ctrl_bw_ch(n0/n1/s0/s1): %d/%d/%d/%d\n",
				 rpt->cnt_swch_bb_ctrl_bw_ch_normal_0_fail,
				 rpt->cnt_swch_bb_ctrl_bw_ch_normal_1_fail,
				 rpt->cnt_swch_bb_ctrl_bw_ch_sense_0_fail,
				 rpt->cnt_swch_bb_ctrl_bw_ch_sense_1_fail);
		PLTFM_MSG_ALWAYS("[WisenseRpt] thermal cnt (task/c2h_fail): %d/%d\n",
				 rpt->cnt_thermal_task,
				 rpt->cnt_thermal_c2h_fail);
		PLTFM_MSG_ALWAYS("[WisenseRpt] ser cnt (dmac/cmac0/cmac1): %d/%d/%d\n",
				 rpt->cnt_ser_dmac,
				 rpt->cnt_ser_cmac0,
				 rpt->cnt_ser_cmac1);
		PLTFM_MSG_ALWAYS("[WisenseRpt] timerhdl cnt: %d\n",
				 rpt->cnt_sense_timer_hdl);
		PLTFM_MSG_ALWAYS("[WisenseRpt] cur_sts cnt : (s0/s1/s2/s3): %d/%d/%d/%d\n",
				 rpt->cnt_wicense_cs_0,
				 rpt->cnt_wicense_cs_1,
				 rpt->cnt_wicense_cs_2,
				 rpt->cnt_wicense_cs_3);
		PLTFM_MSG_ALWAYS("[WisenseRpt] cur_sts cnt : (s4/s5/s6/s7): %d/%d/%d/%d\n",
				 rpt->cnt_wicense_cs_4,
				 rpt->cnt_wicense_cs_5,
				 rpt->cnt_wicense_cs_6,
				 rpt->cnt_wicense_cs_7);
		PLTFM_MSG_ALWAYS("[WisenseRpt] dur_over_th cnt : (wicense/csitxdata): %d/%d\n",
				 rpt->cnt_wicense_dur_over_th,
				 rpt->cnt_csitxdata_dur_over_th);
		PLTFM_MSG_ALWAYS("[WisenseRpt] ---------------\n");
		PLTFM_MSG_ALWAYS("[WisenseRpt] en_dis / wicense active (ms): %d/%d\n",
				 en_dis_time_ms,
				 active_time_ms);
		PLTFM_MSG_ALWAYS("[WisenseRpt] wicense_dur / sw_sensing_ch / sw_normal_ch (us): %d/%d/%d\n",
				 rpt->time_wicense_dur,
				 rpt->time_sw_sensing_ch,
				 rpt->time_sw_normal_ch);
		PLTFM_MSG_ALWAYS("[WisenseRpt] s1/s2/s3/s4/s5 dur (us): %d/%d/%d/%d/%d\n",
				 rpt->time_s1_s2,
				 rpt->time_s2_s3,
				 rpt->time_s3_s4,
				 rpt->time_s4_s5,
				 rpt->time_s5_s6);
		PLTFM_MSG_ALWAYS("[WisenseRpt] <=== report end....\n\n");
	}

	//cp rpt to rpt_all
	rpt_all->error_state |= rpt->error_state;
	rpt_all->cnt_ser_dmac = rpt->cnt_ser_dmac;
	rpt_all->cnt_ser_cmac0 = rpt->cnt_ser_cmac0;
	rpt_all->cnt_ser_cmac1 = rpt->cnt_ser_cmac1;
	rpt_all->en_dis_time_ms = en_dis_time_ms;
	rpt_all->avtive_time_ms = active_time_ms;
	rpt_all->time_wicense_dur = rpt->time_wicense_dur;
	rpt_all->time_sw_sensing_ch = rpt->time_sw_sensing_ch;
	rpt_all->time_sw_normal_ch = rpt->time_sw_normal_ch;
	rpt_all->time_s1_s2 = rpt->time_s1_s2;
	rpt_all->time_s2_s3 = rpt->time_s2_s3;
	rpt_all->time_s3_s4 = rpt->time_s3_s4;
	rpt_all->time_s4_s5 = rpt->time_s4_s5;
	rpt_all->time_s5_s6 = rpt->time_s5_s6;

	//add rpt to rpt_all
	if (rpt->cnt_wicense_enable) {
		//reset rpt_all cnt
		rpt_all->cnt_sense_timer_hdl = rpt->cnt_sense_timer_hdl;
		//rpt_all->cnt_target_tx = rpt->cnt_target_tx;
		rpt_all->cnt_txdata_tx_ok = rpt->cnt_txdata_tx_ok;
		rpt_all->cnt_txdata_tx_fail = rpt->cnt_txdata_tx_fail;

		rpt_all->cnt_txnull_1_send_ok = rpt->cnt_txnull_1_send_ok;
		rpt_all->cnt_txnull_1_send_fail = rpt->cnt_txnull_1_send_fail;
		rpt_all->cnt_txnull_1_tx_ok = rpt->cnt_txnull_1_tx_ok;
		rpt_all->cnt_txnull_1_tx_fail = rpt->cnt_txnull_1_tx_fail;

		rpt_all->cnt_txnull_0_send_ok = rpt->cnt_txnull_0_send_ok;
		rpt_all->cnt_txnull_0_send_fail = rpt->cnt_txnull_0_send_fail;
		rpt_all->cnt_txnull_0_tx_ok = rpt->cnt_txnull_0_tx_ok;
		rpt_all->cnt_txnull_0_tx_fail = rpt->cnt_txnull_0_tx_fail;

		rpt_all->cnt_wicense_enable = rpt->cnt_wicense_enable;
		rpt_all->cnt_wicense_disable = rpt->cnt_wicense_disable;
		rpt_all->cnt_wicense_pause = rpt->cnt_wicense_pause;
		rpt_all->cnt_wicense_resume = rpt->cnt_wicense_resume;

		rpt_all->cnt_swch_ser_stop_fail = rpt->cnt_swch_ser_stop_fail;
		rpt_all->cnt_swch_ser_start_fail = rpt->cnt_swch_ser_start_fail;
		rpt_all->cnt_swch_rx0_idle_poll_fail = rpt->cnt_swch_rx0_idle_poll_fail;
		rpt_all->cnt_swch_rx1_idle_poll_fail = rpt->cnt_swch_rx1_idle_poll_fail;
		rpt_all->cnt_swch_txen_poll_fail = rpt->cnt_swch_txen_poll_fail;
		rpt_all->cnt_swch_rflck_poll_timeout = rpt->cnt_swch_rflck_poll_timeout;
		rpt_all->cnt_swch_force_sensing_dbg = rpt->cnt_swch_force_sensing_dbg;
		rpt_all->cnt_swch_bb_ctrl_bw_ch_normal_0_fail = rpt->cnt_swch_bb_ctrl_bw_ch_normal_0_fail;
		rpt_all->cnt_swch_bb_ctrl_bw_ch_normal_1_fail = rpt->cnt_swch_bb_ctrl_bw_ch_normal_1_fail;
		rpt_all->cnt_swch_bb_ctrl_bw_ch_sense_0_fail = rpt->cnt_swch_bb_ctrl_bw_ch_sense_0_fail;
		rpt_all->cnt_swch_bb_ctrl_bw_ch_sense_1_fail = rpt->cnt_swch_bb_ctrl_bw_ch_sense_1_fail;

		rpt_all->cnt_thermal_task = rpt->cnt_thermal_task;
		rpt_all->cnt_thermal_c2h_fail = rpt->cnt_thermal_c2h_fail;

		rpt_all->cnt_wicense_cs_0 = rpt->cnt_wicense_cs_0;
		rpt_all->cnt_wicense_cs_1 = rpt->cnt_wicense_cs_1;
		rpt_all->cnt_wicense_cs_2 = rpt->cnt_wicense_cs_2;
		rpt_all->cnt_wicense_cs_3 = rpt->cnt_wicense_cs_3;
		rpt_all->cnt_wicense_cs_4 = rpt->cnt_wicense_cs_4;
		rpt_all->cnt_wicense_cs_5 = rpt->cnt_wicense_cs_5;
		rpt_all->cnt_wicense_cs_6 = rpt->cnt_wicense_cs_6;
		rpt_all->cnt_wicense_cs_7 = rpt->cnt_wicense_cs_7;

		rpt_all->cnt_txdata_fail_rtydrop = rpt->cnt_txdata_fail_rtydrop;
		rpt_all->cnt_txdata_fail_lifedrop = rpt->cnt_txdata_fail_lifedrop;
		rpt_all->cnt_txdata_fail_maciddrop = rpt->cnt_txdata_fail_maciddrop;
		rpt_all->cnt_txdata_fail_invalidrpt = rpt->cnt_txdata_fail_invalidrpt;

		rpt_all->cnt_txdata_send_ok = rpt->cnt_txdata_send_ok;
		rpt_all->cnt_txdata_send_fail = rpt->cnt_txdata_send_fail;
		rpt_all->cnt_wicense_dur_over_th = rpt->cnt_wicense_dur_over_th;
		rpt_all->cnt_csitxdata_dur_over_th = rpt->cnt_csitxdata_dur_over_th;
	} else {
		//add rpt_all cnt
		rpt_all->cnt_sense_timer_hdl += rpt->cnt_sense_timer_hdl;
		//rpt_all->cnt_target_tx += rpt->cnt_target_tx;
	...
	if (rpt->cnt_wicense_disable > 0 || rpt->cnt_wicense_pause > 0) {
		txdata_fail_rate = (100 * rpt_all->cnt_txdata_tx_fail) / (rpt_all->cnt_txdata_tx_fail + rpt_all->cnt_txdata_tx_ok);

		PLTFM_MSG_ALWAYS("[WisenseRpt] ===> Summary report start (pause or disable)....\n");
		PLTFM_MSG_ALWAYS("[WisenseRpt] error state: 0x%x\n", rpt_all->error_state);
		PLTFM_MSG_ALWAYS("[WisenseRpt] cnt (enable/pause/resume/disable): %d/%d/%d/%d/%d\n",
				 rpt_all->cnt_wicense_enable,
				 rpt_all->cnt_wicense_pause,
				 rpt_all->cnt_wicense_resume,
				 rpt_all->cnt_wicense_disable,
				 txdata_fail_rate);
		PLTFM_MSG_ALWAYS("[WisenseRpt] txdata cnt (send ok/send fail/tx ok/tx fail/Tx Fail Rate): %d/%d/%d/%d\n",
				 rpt_all->cnt_txdata_send_ok,
				 rpt_all->cnt_txdata_send_fail,
				 rpt_all->cnt_txdata_tx_ok,
				 rpt_all->cnt_txdata_tx_fail);
		PLTFM_MSG_ALWAYS("[WisenseRpt] txdata fail ret cnt (rty/life/macid/invalid rpt): %d/%d/%d/%d\n",
				 rpt_all->cnt_txdata_fail_rtydrop,
				 rpt_all->cnt_txdata_fail_lifedrop,
				 rpt_all->cnt_txdata_fail_maciddrop,
				 rpt_all->cnt_txdata_fail_invalidrpt);
		PLTFM_MSG_ALWAYS("[WisenseRpt] txnull_1 cnt (send ok/send fail/tx ok/tx fail): %d/%d/%d/%d\n",
				 rpt_all->cnt_txnull_1_send_ok,
				 rpt_all->cnt_txnull_1_send_fail,
				 rpt_all->cnt_txnull_1_tx_ok,
				 rpt_all->cnt_txnull_1_tx_fail);
		PLTFM_MSG_ALWAYS("[WisenseRpt] txnull_0 cnt (send ok/send fail/tx ok/tx fail): %d/%d/%d/%d\n",
				 rpt_all->cnt_txnull_0_send_ok,
				 rpt_all->cnt_txnull_0_send_fail,
				 rpt_all->cnt_txnull_0_tx_ok,
				 rpt_all->cnt_txnull_0_tx_fail);
		PLTFM_MSG_ALWAYS("[WisenseRpt] swch fail cnt (ser stop/ser start/rflck to/force_ch): %d/%d/%d/%d\n",
				 rpt_all->cnt_swch_ser_stop_fail,
				 rpt_all->cnt_swch_ser_start_fail,
				 rpt_all->cnt_swch_rflck_poll_timeout,
				 rpt_all->cnt_swch_force_sensing_dbg);
		PLTFM_MSG_ALWAYS("[WisenseRpt] swch fail cnt (rx0 idle/rx1 idle/txen): %d/%d/%d\n",
				 rpt_all->cnt_swch_rx0_idle_poll_fail,
				 rpt_all->cnt_swch_rx1_idle_poll_fail,
				 rpt_all->cnt_swch_txen_poll_fail);
		PLTFM_MSG_ALWAYS("[WisenseRpt] swch fail cnt :ctrl_bw_ch(n0/n1/s0/s1): %d/%d/%d/%d\n",
				 rpt_all->cnt_swch_bb_ctrl_bw_ch_normal_0_fail,
				 rpt_all->cnt_swch_bb_ctrl_bw_ch_normal_1_fail,
				 rpt_all->cnt_swch_bb_ctrl_bw_ch_sense_0_fail,
				 rpt_all->cnt_swch_bb_ctrl_bw_ch_sense_1_fail);
		PLTFM_MSG_ALWAYS("[WisenseRpt] thermal cnt (task/c2h_fail): %d/%d\n",
				 rpt_all->cnt_thermal_task,
				 rpt_all->cnt_thermal_c2h_fail);
		PLTFM_MSG_ALWAYS("[WisenseRpt] ser cnt (dmac/cmac0/cmac1): %d/%d/%d\n",
				 rpt_all->cnt_ser_dmac,
				 rpt_all->cnt_ser_cmac0,
				 rpt_all->cnt_ser_cmac1);
		PLTFM_MSG_ALWAYS("[WisenseRpt] timerhdl cnt: %d\n",
				 rpt_all->cnt_sense_timer_hdl);
		PLTFM_MSG_ALWAYS("[WisenseRpt] cur_sts cnt : (s0/s1/s2/s3): %d/%d/%d/%d\n",
				 rpt_all->cnt_wicense_cs_0,
				 rpt_all->cnt_wicense_cs_1,
				 rpt_all->cnt_wicense_cs_2,
				 rpt_all->cnt_wicense_cs_3);
		PLTFM_MSG_ALWAYS("[WisenseRpt] cur_sts cnt : (s4/s5/s6/s7): %d/%d/%d/%d\n",
				 rpt_all->cnt_wicense_cs_4,
				 rpt_all->cnt_wicense_cs_5,
				 rpt_all->cnt_wicense_cs_6,
				 rpt_all->cnt_wicense_cs_7);
		PLTFM_MSG_ALWAYS("[WisenseRpt] dur_over_th cnt : (wicense/csitxdata): %d/%d\n",
				 rpt_all->cnt_wicense_dur_over_th,
				 rpt_all->cnt_csitxdata_dur_over_th);
		PLTFM_MSG_ALWAYS("[WisenseRpt] ---------------\n");
		PLTFM_MSG_ALWAYS("[WisenseRpt] en_dis / wicense active (ms): %d/%d\n",
				 en_dis_time_ms,
				 active_time_ms);
		PLTFM_MSG_ALWAYS("[WisenseRpt] wicense_dur / sw_sensing_ch / sw_normal_ch (us): %d/%d/%d\n",
				 rpt_all->time_wicense_dur,
				 rpt_all->time_sw_sensing_ch,
				 rpt_all->time_sw_normal_ch);
		PLTFM_MSG_ALWAYS("[WisenseRpt] s1/s2/s3/s4/s5 dur (us): %d/%d/%d/%d/%d\n",
				 rpt_all->time_s1_s2,
				 rpt_all->time_s2_s3,
				 rpt_all->time_s3_s4,
				 rpt_all->time_s4_s5,
				 rpt_all->time_s5_s6);
		PLTFM_MSG_ALWAYS("[WisenseRpt] <=== Summary report end....\n\n");
	}
