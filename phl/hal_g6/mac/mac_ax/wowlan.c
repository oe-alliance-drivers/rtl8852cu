			 ...
			 struct rtw_dev2hst_gpio_info *parm)
{
	...
	if (parm->gpio_num > adapter->gpio_info.gpio_max) {
		PLTFM_MSG_ERR("gpio num %d > %d\n", parm->gpio_num, adapter->gpio_info.gpio_max);
		return MACNOITEM;
	}
		...
		, enum rtw_fw_type fw_type)
{
	...
	h2c_info.agg_en = 0;
	h2c_info.content_len = sizeof(struct rtw_hal_mac_apf_ctrl);
	h2c_info.h2c_cat = FWCMD_H2C_CAT_MAC;
	...
	if (fw_type != RTW_FW_WOWLAN && papf_ofld_ctrl->apf_en != APF_REPORT) {
		// R_AX_DATA_FLTR
		val32 = MAC_REG_R32(R_AX_DATA_FLTR);
		val32 &= ~(B_AX_A_DATA8_MSK << B_AX_A_DATA8_SH);
		if (papf_ofld_ctrl->apf_en == APF_DISABLE)
			val32 |= (RX_TO_HOST << B_AX_A_DATA8_SH);
		else // APF_ENABLE
			val32 |= (RX_TO_WLCPU << B_AX_A_DATA8_SH);
		MAC_REG_W32(R_AX_DATA_FLTR, val32);
	}
