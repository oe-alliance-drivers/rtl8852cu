...
static u32 _patch_snd_mu_err(struct mac_ax_adapter *adapter, u8 band)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
	u16 cr;

	if (chk_patch_snd_mu_err(adapter) == PATCH_DISABLE)
		return MACSUCCESS;

	cr = band ? R_AX_BFMEE_RESP_OPTION_C1 : R_AX_BFMEE_RESP_OPTION;
	val32 = MAC_REG_R32(cr);
	val32 = SET_CLR_WORD(val32, PATCH_BFRP_RX_STANDBY_TIMER, B_AX_BFMEE_BFRP_RX_STANDBY_TIMER);
	MAC_REG_W32(cr, val32);
...
u32 mac_init_snd_mee(struct mac_ax_adapter *adapter, u8 band)
{
	...
	if (is_chip_id(adapter, MAC_AX_CHIP_ID_8852A) ||
		...
		val32 = SET_CLR_WORD(val32, NDP_RX_STANDBY_TIMER,
				     B_AX_BFMEE_NDP_RX_STANDBY_TIMER);
	...
	if (is_chip_id(adapter, MAC_AX_CHIP_ID_8852C) ||
		...
		val32 = MAC_REG_R32(band ? R_AX_BFMEE_RESP_OPTION_C1 :
				    R_AX_BFMEE_RESP_OPTION);
		val32 = SET_CLR_WORD(val32, NDP_RX_STANDBY_TIMER, B_AX_BFMEE_NDP_RX_TIMEOUT);
		val32 |= (B_AX_BFMEE_HT_NDPA_EN | B_AX_BFMEE_VHT_NDPA_EN |
			  B_AX_BFMEE_HE_NDPA_EN);
		MAC_REG_W32(band ? R_AX_BFMEE_RESP_OPTION_C1 :
				    R_AX_BFMEE_RESP_OPTION, val32);
	...
	_patch_snd_mu_err(adapter, band);
