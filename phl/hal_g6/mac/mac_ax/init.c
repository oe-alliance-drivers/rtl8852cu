...
u32 mac_ax_init_state(struct mac_ax_adapter *adapter)
{
	...
	adapter->sm = sm;
#if MAC_AX_FEATURE_DBGPKG
	for (i = 0; i < H2CC2H_PATH_DEBUG_END; i++) {
		adapter->fw_info.h2c_path_record[i].host_tx = 0xffffffff;
		adapter->fw_info.h2c_path_record[i].host_rx = 0xffffffff;
		adapter->fw_info.h2c_path_record[i].wlancpu_rx = 0xffffffff;
		adapter->fw_info.h2c_path_record[i].wlancpu_tx = 0xffffffff;
	}
#endif /* MAC_AX_FEATURE_DBGPKG */
