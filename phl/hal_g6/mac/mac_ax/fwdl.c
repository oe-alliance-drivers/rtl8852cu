...
static u32 fwdl_blacklist_check(struct mac_ax_adapter *adapter, u8 *content)
{
	u8 fw_ver_img = content[FWDL_SECURE_SECTION_FW_VER_DRV];
	u8 bl_ver_img = content[FWDL_SECURE_SECTION_BL_VER_DRV];
	u8 byteidx = fw_ver_img >> 3;
	u8 bitidx = 0x1 << (fw_ver_img & 0x7);
	u8 *bl_list = NULL;
	u8 bl_ver_drv = 0;
	...
	if (FWDL_IS_8852B) {
		...
		bl_list = fw_blacklist_8852B;
		bl_ver_drv = FW_BLACKLIST_VERSION_8852B;
#endif
	} else if (FWDL_IS_8852C) {
#if defined(MAC_8852C_SUPPORT)
		bl_list = fw_blacklist_8852C;
		bl_ver_drv = FW_BLACKLIST_VERSION_8852C;
#endif
	} else if (FWDL_IS_8192XB) {
#if defined(MAC_8192XB_SUPPORT)
		bl_list = fw_blacklist_8192XB;
		bl_ver_drv = FW_BLACKLIST_VERSION_8192XB;
#endif
	} else if (FWDL_IS_8851B) {
#if  defined(MAC_8851B_SUPPORT)
		bl_list = fw_blacklist_8851B;
		bl_ver_drv = FW_BLACKLIST_VERSION_8851B;
#endif
	} else if (FWDL_IS_8852D) {
#if defined(MAC_8852D_SUPPORT)
		bl_list = fw_blacklist_8852D;
		bl_ver_drv = FW_BLACKLIST_VERSION_8852D;
#endif
	} else if (FWDL_IS_8852BT) {
#if defined(MAC_8852BT_SUPPORT)
		bl_list = fw_blacklist_8852BT;
		bl_ver_drv = FW_BLACKLIST_VERSION_8852BT;
#endif
	} else {
		PLTFM_MSG_ERR("[FWDL][ERR] Unsupported Chip ID\n");
		return MACNOITEM;
	}

	if (!bl_list) {
		PLTFM_MSG_ERR("[FWDL][ERR] Compiler Flag Unsupported Chip ID\n");
		return MACNOITEM;
	}

	if (bl_ver_img > bl_ver_drv) {
		PLTFM_MSG_ERR("[FWDL][ERR] Driver Secure Version Out of Date!\n");
		return MACFWSECBOOT;
	}

	if (bl_list[byteidx] & bitidx) {
		PLTFM_MSG_ERR("[FWDL][ERR] Anti-Rollback Check fail!, fw_ver = %d\n", fw_ver_img);
		return MACFWSECBOOT;
	}
			...
			struct fw_bin_info *info)
{
	...
	for (i = 0; i < info->section_num; i++) {
		...
		if (cur_section_info->type == FWDL_SECURITY_SECTION_TYPE) {
			ret = fwdl_blacklist_check(adapter, cur_section_info->addr);
			if (ret)
				return ret;
		}
		if (cur_section_info->type == FWDL_SECURITY_SECTION_TYPE &&
			    ...
			    u8 *fw, u32 hdr_len, u8 redl)
{
	u8 *buf;
	u32 ret = MACSUCCESS;
	u32 h2cb_alloc_fail_cnt = 0;
	u32 h2cb_push_fail_cnt = 0;
	u32 h2c_pkt_set_hdr_fail_cnt = 0;
	u32 h2c_pkt_build_txd_fail_cnt = 0;
	u32 PLTFM_TX_fail_cnt = 0;
#if MAC_AX_PHL_H2C
...
#endif
	while (1) {
		if (h2cb_alloc_fail_cnt > FWDL_SECTION_DL_FAIL_COUNT ||
		    h2cb_push_fail_cnt > FWDL_SECTION_DL_FAIL_COUNT ||
		    h2c_pkt_set_hdr_fail_cnt > FWDL_SECTION_DL_FAIL_COUNT ||
		    h2c_pkt_build_txd_fail_cnt > FWDL_SECTION_DL_FAIL_COUNT ||
		    PLTFM_TX_fail_cnt > FWDL_SECTION_DL_FAIL_COUNT) {
			PLTFM_MSG_ERR("[FWDL][ERR][%s] __sections_download__ fail\n", __func__);
			PLTFM_MSG_ERR("[FWDL][ERR][%s] h2cb_alloc_fail_cnt = %d\n", __func__,
				      h2cb_alloc_fail_cnt);
			PLTFM_MSG_ERR("[FWDL][ERR][%s] h2cb_push_fail_cnt = %d\n", __func__,
				      h2cb_push_fail_cnt);
			PLTFM_MSG_ERR("[FWDL][ERR][%s] h2c_pkt_set_hdr_fail_cnt = %d\n", __func__,
				      h2c_pkt_set_hdr_fail_cnt);
			PLTFM_MSG_ERR("[FWDL][ERR][%s] h2c_pkt_build_txd_fail_cnt = %d\n", __func__,
				      h2c_pkt_build_txd_fail_cnt);
			PLTFM_MSG_ERR("[FWDL][ERR][%s] PLTFM_TX_fail_cnt = %d\n", __func__,
				      PLTFM_TX_fail_cnt);
			return MACPTMTXFAIL;
		}
		...
		if (!h2cb) {
			h2cb_alloc_fail_cnt++;
			PLTFM_MSG_ERR("[FWDL][ERR][%s] h2cb_alloc fail %d times\n", __func__,
				      h2cb_alloc_fail_cnt);
#if MAC_AX_PHL_H2C
		...
		if (!buf) {
			h2cb_push_fail_cnt++;
			PLTFM_MSG_ERR("[FWDL][ERR][%s] h2cb_put fail %d times\n", __func__,
				      h2cb_push_fail_cnt);
#if MAC_AX_PHL_H2C
		...
		if (redl) {
			ret = h2c_pkt_set_hdr_fwdl(adapter, h2cb, FWCMD_TYPE_H2C, FWCMD_H2C_CAT_MAC,
						   FWCMD_H2C_CL_FWDL, FWCMD_H2C_FUNC_FWHDR_REDL, 0,
						   0);
		} else {
			ret = h2c_pkt_set_hdr_fwdl(adapter, h2cb, FWCMD_TYPE_H2C, FWCMD_H2C_CAT_MAC,
						   FWCMD_H2C_CL_FWDL, FWCMD_H2C_FUNC_FWHDR_DL, 0,
						   0);
		}

		if (ret) {
			h2c_pkt_set_hdr_fail_cnt++;
			PLTFM_MSG_ERR("[FWDL][ERR][%s] set h2c hdr fail %d times\n", __func__,
				      h2c_pkt_set_hdr_fail_cnt);
#if MAC_AX_PHL_H2C
			PLTFM_RECYCLE_H2C(h2cb);
#else
		...
		if (ret) {
			h2c_pkt_build_txd_fail_cnt++;
			PLTFM_MSG_ERR("[FWDL][ERR][%s] h2c_pkt_build_txd fail %d times\n",
				      __func__, h2c_pkt_build_txd_fail_cnt);
#if MAC_AX_PHL_H2C
			PLTFM_RECYCLE_H2C(h2cb);
#else
			h2cb_free(adapter, h2cb);
		...
		if (ret) {
			PLTFM_TX_fail_cnt++;
			PLTFM_MSG_ERR("[FWDL][ERR][%s] PLTFM_TX fail %d times\n", __func__,
				      PLTFM_TX_fail_cnt);
			PLTFM_DELAY_MS(1);
			continue;
		}
			       ...
			       struct fwhdr_section_info *info)
{
	...
	u32 h2cb_alloc_fail_cnt = 0;
	u32 h2cb_push_fail_cnt = 0;
	u32 section_built_txd_fail_cnt = 0;
	u32 PLTFM_TX_fail_cnt = 0;
#if MAC_AX_PHL_H2C
	struct rtw_h2c_pkt *h2cb;
#else
	...
	while (residue_len) {
		if (h2cb_alloc_fail_cnt > FWDL_SECTION_DL_FAIL_COUNT ||
		    h2cb_push_fail_cnt > FWDL_SECTION_DL_FAIL_COUNT ||
		    section_built_txd_fail_cnt > FWDL_SECTION_DL_FAIL_COUNT ||
		    PLTFM_TX_fail_cnt > FWDL_SECTION_DL_FAIL_COUNT) {
			PLTFM_MSG_ERR("[FWDL][ERR][%s] __sections_download__ fail\n", __func__);
			PLTFM_MSG_ERR("[FWDL][ERR][%s] h2cb_alloc_fail_cnt = %d\n", __func__,
				      h2cb_alloc_fail_cnt);
			PLTFM_MSG_ERR("[FWDL][ERR][%s] h2cb_push_fail_cnt = %d\n", __func__,
				      h2cb_push_fail_cnt);
			PLTFM_MSG_ERR("[FWDL][ERR][%s] section_built_txd_fail_cnt = %d\n", __func__,
				      section_built_txd_fail_cnt);
			PLTFM_MSG_ERR("[FWDL][ERR][%s] PLTFM_TX_fail_cnt = %d\n", __func__,
				      PLTFM_TX_fail_cnt);
			return MACPTMTXFAIL;
		}
		...
		if (ret) {
			section_built_txd_fail_cnt++;
			PLTFM_MSG_ERR("[FWDL][ERR][%s] __sections_build_txd fail %d times\n",
				      __func__, section_built_txd_fail_cnt);
#if MAC_AX_PHL_H2C
			PLTFM_RECYCLE_H2C(h2cb);
#else
			h2cb_free(adapter, h2cb);
		...
		fw_section_id++;
		sprintf_s(file_name, 56, "b_section%2d.bin", fw_section_id);

		memset(wszClassName, 0, sizeof(wszClassName));
		MultiByteToWideChar(CP_ACP, 0, file_name, (int)strlen(file_name) + 1,
				    wszClassName, ARRAY_SIZE(wszClassName));
		hFile = CreateFile(wszClassName, // name of the write
				   CREATE_ALWAYS,          // open for writing
				   0,                      // do not share
		...
		if (hFile == INVALID_HANDLE_VALUE)
			PLTFM_MSG_ERR("[FWDL][ERR][%s] create file fail\n", __func__);

		WriteFile(hFile,           // open file handle
			  h2cb->data,      // start of data to write
			  h2cb->len,  // number of bytes to write
...
static u32 fwdl_phase0(struct mac_ax_adapter *adapter)
{
	...
	while (--cnt) {
		...
		if (val8 == 0) {
			read_zero_count++;
			if (read_zero_count >= FWDL_READ_ZERO_COUNT) {
				PLTFM_MSG_ERR("[ERR]%s: read zero count exceed %d\n",
						__func__, FWDL_READ_ZERO_COUNT);
				return MACNOITEM;
			}
		} else {
			read_zero_count = 0;
		}
		if (val8 & B_AX_H2C_PATH_RDY)
			break;
		PLTFM_DELAY_US(1);
	}
...
#endif
		if (cur_bootstep == 0 && cur_secure_step == 0) {
			read_zero_count++;
			if (read_zero_count >= FWDL_READ_ZERO_COUNT) {
				PLTFM_MSG_ERR("[ERR]%s: read zero count exceed %d\n",
						__func__, FWDL_READ_ZERO_COUNT);
