			     ...
			     struct lps_parm *parm)
{
	...
	h2c_info.rec_ack = 0;
	if (parm->psmode == MAC_AX_PS_MODE_ACTIVE) {
		h2c_info.done_ack = 1;
		adapter->sm.ps_state = MAC_AX_PS_STS_WAIT;
	} else {
		h2c_info.done_ack = 0;
		adapter->sm.ps_state = MAC_AX_PS_STS_PS;
	}
...
u32 mac_chk_leave_lps(struct mac_ax_adapter *adapter, u8 macid)
{
	...
	if (adapter->sm.ps_state != MAC_AX_PS_STS_ACTIVE)
		return MACPROCBUSY;
