...
enum rtw_hal_status rtw_hal_efuse_get_block_map_by_range(
    struct hal_info_t *hal_info, u16 offset, u8 *value, u16 len)
{
	enum rtw_hal_status status = RTW_HAL_STATUS_FAILURE;

	status = rtw_efuse_get_block_map_by_range(hal_info->efuse, offset,
						  value, len);
