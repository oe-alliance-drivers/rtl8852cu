...
static u8 _diag_bb_type_mapping(u8 bb_type)
{
	...
	case F_DIG:
		...
		break;
	case F_EDCCA:
		phl_diag_bb_type = PHL_DIAG_BB_EVT_EDCCA;
		break;
	default:
...
void rtw_hal_bb_query_rx_rate_plurality(struct rtw_hal_com_t *hal_com, u16 *info, enum phl_phy_idx phy_idx)
{
	struct hal_info_t *hal_info = hal_com->hal_priv;
	struct bb_bkp_phy_utility_info rpt = {0};

	halbb_query_phy_utility_info(hal_info->bb, &rpt, phy_idx);
	*info  = rpt.rx_rate_plurality;
}
...
void rtw_hal_bb_query_rx_rate_plurality(struct rtw_hal_com_t *hal_com, u16 *info,
                                        enum phl_phy_idx phy_idx)
{
}
