...
void efuse_shadow_read_by_range(struct efuse_t *efuse, u16 offset, u8 *value,
				u16 len)
{
	struct rtw_hal_com_t *hal_com = (struct rtw_hal_com_t *)efuse->hal_com;

	_os_mem_cpy(hal_com->drv_priv, value, (efuse->shadow_map + offset),
		    len);
}
...
enum rtw_hal_status rtw_efuse_get_block_map_by_range(void *efuse, u16 offset,
						     u8 *value, u16 len)
{
	struct efuse_t *efuse_info = efuse;

	efuse_shadow_read_by_range(efuse_info, offset, value, len);
