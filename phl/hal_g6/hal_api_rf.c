...
#define HAL_RF_DIAG_RFCR_CUR_VER 1
enum rtw_hal_status rtw_hal_rf_query_diag_rfcr(
	void *hal, u8 *buf, u8 ver)
{
#if 0
	struct hal_info_t *hal_info = NULL;
	struct halrf_arfc_info_v1 rfcr = {0};
	struct rtw_phl_diag_rfcr_v1 *output = NULL;
	u8 i = 0;
	...
	/* check ver */
	if (ver != HAL_RF_DIAG_RFCR_CUR_VER) {
		PHL_ERR("%s fail, mismatch ver(%d, tgt %d)\n",
			__func__, ver, HAL_RF_DIAG_RFCR_CUR_VER);
		return RTW_HAL_STATUS_FAILURE;
	}
	hal_info = (struct hal_info_t *)hal;
	output = (struct rtw_phl_diag_rfcr_v1 *)buf;

	/* check query result */
	output->is_valid = false;
	rfcr = halrf_query_arfc_sta(hal_info->rf);
	if (rfcr.is_valid) {
		output->is_valid = true;
		output->rfcr_err =
			(rfcr.rf_mode_ok ? 0 : PHL_RFCR_ERR_MODE) |
			(rfcr.rf_ch_ok ? 0 : PHL_RFCR_ERR_CH) |
			(rfcr.rf_hwsi_ok ? 0 : PHL_RFCR_ERR_HWSI) |
			(rfcr.rf_radio_ok ? 0 : PHL_RFCR_ERR_RADIO);
		for (i = 0; i < 2; i++) {
			output->rf_0x0[i] = rfcr.rf_0x0[i];
			output->rf_0x3[i] = rfcr.rf_0x3[i];
			output->rf_0x5[i] = rfcr.rf_0x5[i];
			output->rf_0x18[i] = rfcr.rf_0x18[i];
			output->rf_radio_ver[i] = rfcr.rf_radio_ver[i];
		}
	} else {
		PHL_WARN("%s is_valid = false\n", __func__);
	}
