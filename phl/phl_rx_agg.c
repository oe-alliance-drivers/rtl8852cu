...
#ifdef CONFIG_PHL_TID_RX_DROP_INFO	
	_os_mem_set(drv_priv, r->drop_info, 0,
	            RX_DROP_RSN_MAX * MAX_PHL_RX_DROP_INFO_RECORED *
	            sizeof(struct phl_tid_rx_drop_info));
	_os_mem_set(drv_priv, r->drop_info_idx, 0,
	            RX_DROP_RSN_MAX * sizeof(u8));
	for (rsn = RX_DROP_RSN_MIN; rsn < RX_DROP_RSN_MAX; rsn++) {
		for (idx = 0; idx < MAX_PHL_RX_DROP_INFO_RECORED; idx++) {
			r->drop_info[rsn][idx].idx = idx;
			r->drop_info[rsn][idx].rsn = rsn;
		}
	}
