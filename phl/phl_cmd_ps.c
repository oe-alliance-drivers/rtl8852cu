...
static void _dump_ps_cap(struct cmd_ps *ps)
{
	...
	PHL_INFO("[PS_CMD], leave_fail_th: 0x%x\n", ps_cap->leave_fail_th);
	...
	PHL_INFO("[PS_CMD], lps_adv_cap: 0x%x\n", ps_cap->lps_adv_cap);
	PHL_INFO("[PS_CMD], bcn_tracking: %d\n", ps_cap->bcn_tracking);
...
static void _leave_success_hdlr(struct cmd_ps *ps)
{
	...
	ps->leave_fail_cnt = 0; /* reset */
}

#ifdef CONFIG_PHL_DIAGNOSE
static void _ntfy_leave_ps_failure(struct cmd_ps *ps)
{
	if (!ps)
		return;

	if (ps->ps_mode == PS_MODE_NONE) {
		PHL_WARN("[PS_CMD], %s(): unexpected ps mode \n", __func__);
		return;
	}

	/* only notify mac, only one band is necessary */
	rtw_hal_notification_ex(ps->phl_info->hal,
		                MSG_EVT_PS_LEAVE_FAILURE,
		                false, true, false,
		                HW_BAND_0);
}
#endif
...
static enum phl_mdl_ret_code _leave_fail_hdlr(struct cmd_ps *ps)
{
	...
	_ntfy_leave_ps_failure(ps);
#endif

	ps->leave_fail_cnt++;

	PHL_WARN("[PS_CMD], %s(): action 0x%x, leave_fail_cnt %d\n",
	         __func__, ps_cap->leave_fail_act, ps->leave_fail_cnt);
	...
	if (ps_cap->leave_fail_act & PS_LEAVE_FAIL_ACT_L2) {
		if (ps->leave_fail_cnt == ps_cap->leave_fail_th)
                                    ...
                                    u16 *macid)
{
	...
	if (target_mode == PS_MODE_IPS) {
		...
		if (ps->app_info.sta_assoc_gohotspot == true) {
			PHL_TRACE(COMP_PHL_PS, _PHL_INFO_, "[PS_CMD], %s(): sta assoc go hotspot.\n", __func__);
			return false;
		}
...
static bool _chk_ps_cap(struct cmd_ps *ps, u8 mode)
{
	...
	case PS_MODE_LPS:
		if (ps_cap->lps_en == PS_OP_MODE_DISABLED) {
			...
			/* force enable */
			if (!TEST_STATUS_FLAG(ps->rt_stop_rson, PS_RT_DEBUG))
				return true;
...
static void _ps_watchdog_info_dump(struct cmd_ps *ps)
{
	...
	PHL_TRACE(COMP_PHL_PS, _PHL_INFO_, "req idle cnt(%d), req busy cnt(%d), defer ps(%s)\n",
	          ps->req_idle_q.cnt, ps->req_busy_q.cnt, (_ps_chk_defer(ps) ? "yes" : "no"));
	...
	if (ps_cap->defer_para.defer_rson) {
		PHL_TRACE(COMP_PHL_PS, _PHL_INFO_, "tx ping pass time(%dms), tx dhcp pass time(%dms) \n",
...
static void _ps_mr_info_upt(struct cmd_ps *ps, struct rtw_wifi_role_t *role)
{
	...
	sta = rtw_phl_get_stainfo_self(ps->phl_info, &role->rlink[RTW_RLINK_PRIMARY]);
	ps->app_info.sta_assoc_gohotspot = sta->assoc_gohotspot;

	PHL_TRACE(COMP_PHL_PS, _PHL_INFO_, "[PS_CMD], %s(): gc_active %d, ap_active %d\n", __func__,
				ps->mr_info.gc_active, ps->mr_info.ap_active);
	PHL_TRACE(COMP_PHL_PS, _PHL_INFO_, "[PS_CMD], %s(): sta_assoc_gohotspot %d\n", __func__,
				ps->app_info.sta_assoc_gohotspot);
}
...
_ps_mdl_query_info(void *dispr, void *priv, struct phl_module_op_info *info)
{
	...
	case PS_MDL_OP_BASIC_INFO:
		...
		basic_info->leave_fail_cnt = ps->leave_fail_cnt;
		basic_info->recy_cnt = ps->recy_info.recy_cnt;
		basic_info->assoc_gohotspot = ps->app_info.sta_assoc_gohotspot;
