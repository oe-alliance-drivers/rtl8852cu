...
enum tid_rx_drop_rsn {
	RX_DROP_OUT_OF_DATE = 0,
	RX_DROP_ALREADY_STORED,
	RX_DROP_IV_REPLAY,
	RX_DROP_OUT_OF_WINDOW,
	RX_DROP_RSN_END
};

#define RX_DROP_RSN_MIN RX_DROP_OUT_OF_DATE
#define RX_DROP_RSN_MAX RX_DROP_RSN_END

enum tid_rx_drop_status {
	RX_DROP_INFO_STS_IDLE,
	RX_DROP_INFO_STS_DROPPING
};

#define MAX_PHL_RX_DROP_INFO_RECORED 5
struct phl_tid_rx_drop_info {
	u8 idx; /* record idx */
	u8 rsn; /* enum tid_rx_drop_rsn */
	u8 sts; /* enum tid_rx_drop_status */
	u16 tid;
	u16 cnt; /* drop cnt before recover */
	u32 start_time; /* start time of the first dropped packet */
	u16 drop_start_seq; /* seq number of the first dropped packet */
	u16 drop_recover_seq; /* seq number of the recovered packet not dropped */
	u32 duration; /* duration of continuous drop */
#ifdef CONFIG_PHL_DIAGNOSE
	u32 last_indic_duration;
	u16 last_indic_drop_cnt;
#endif
};
...
#ifdef CONFIG_PHL_TID_RX_DROP_INFO
	struct phl_tid_rx_drop_info drop_info[RX_DROP_RSN_MAX][MAX_PHL_RX_DROP_INFO_RECORED];
	u8 drop_info_idx[RX_DROP_RSN_MAX];
#endif
};
