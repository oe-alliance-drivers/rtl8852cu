...
static enum rtw_phl_status phl_mp_efuse_wifi_get_block_map_by_range(
	struct mp_context *mp, struct mp_efuse_arg *arg)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;

	hal_status = rtw_hal_mp_efuse_get_block_by_range(mp, arg);
...
enum rtw_phl_status mp_efuse(struct mp_context *mp, struct mp_efuse_arg *arg)
{
	...
	case MP_EFUSE_CMD_BT_GET_MASK_BUF:
		...
		break;
	case MP_EFUSE_CMD_WIFI_GET_BLOCK_MAP_BY_RANGE:
		PHL_INFO("%s: CMD = MP_EFUSE_CMD_WIFI_GET_BLOCK_MAP_BY_RANGE\n",
			 __FUNCTION__);
			 phl_status = phl_mp_efuse_wifi_get_block_map_by_range(mp, arg);
		break;
	default:
