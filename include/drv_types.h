...
#define MAX_IFACE_LINK_BMP_SIZE (CONFIG_IFACE_NUMBER) /* this branch support one link only */
...
struct rf_ctl_t {
	...
	#ifdef CONFIG_AP_MODE
	_lock country_ie_lock[BAND_MAX];
	u8 country_ie[BAND_MAX + 1][256]; /* last one is temporary buffer */
	#endif
#endif
...
#ifdef CONFIG_ECSA
#define IS_ECSA_RUNNING(adapter) rtw_mr_is_ecsa_running(adapter)
#else
#define IS_ECSA_RUNNING(adapter) 0
#endif /* CONFIG_ECSA */

#define DEV_IS_ECSA_RUNNING(dvobj, hwband) IS_ECSA_RUNNING(dvobj_get_primary_adapter(dvobj))
...
#define ALINK_GET_ORI_BAND(alink) ((alink)->mlmepriv.ori_chandef.band)
#define ALINK_GET_ORI_CH(alink) ((alink)->mlmepriv.ori_chandef.chan)
#define ALINK_GET_ORI_BW(alink) ((alink)->mlmepriv.ori_chandef.bw)
#define ALINK_GET_ORI_OFFSET(alink) ((alink)->mlmepriv.ori_chandef.offset)

#define ALINK_GET_EXT_NETWORK(alink) (&(alink)->mlmeextpriv.mlmext_info.network)

static inline bool alink_is_on_hwband(struct _ADAPTER_LINK *alink, enum phl_band_idx hwband)
{
	return ALINK_GET_HWBAND(alink) == hwband;
}
